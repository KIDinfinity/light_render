
import request from '@/utils/request';

export async function assignTask(params?: any, option?: any): Promise<any> {
  return request('/api/bpm/systemsupport/assignTask', {
    ...option,
    method: 'POST',
    body: params,
  });
}

export async function completeTask(params?: any, option?: any): Promise<any> {
  return request('/api/bpm/systemsupport/completeTask', {
    ...option,
    method: 'POST',
    body: params,
  });
}

export async function startProcessInstance(params?: any, option?: any): Promise<any> {
  return request('/api/bpm/systemsupport/startProcessInstance', {
    ...option,
    method: 'POST',
    body: params,
  });
}

export default {
  assignTask,
  completeTask,
  startProcessInstance,
}
