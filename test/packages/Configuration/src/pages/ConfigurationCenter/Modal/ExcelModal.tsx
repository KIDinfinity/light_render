...
function ExcelModal() {
  ...
  const showExcelModal: boolean = useSelector(
    (state: any) => state.configurationExcel.showExcelModal
  );
  const currentMenu: CurrentMenuProps = useSelector(
    (state: any) => state.configurationMenu.currentMenu
  );
  const excelData: ExcelDataProps = useSelector((state: any) => state.configurationExcel.excelData);
  const importErrors: ExcelDataProps = useSelector(
    (state: any) => state.configurationExcel.importErrors
  );
  const currentTab: string = useSelector((state: any) => state.configurationTabs.currentTab);
  const loadingImport: boolean = useSelector(
    (state: any) => state.loading.effects['configurationExcel/importData']
  );
  const loadingImportDataVersion: boolean = useSelector(
    (state: any) => state.loading.effects['configurationExcel/importDataByVersion']
  );
  ...
  const { id: functionId, dataImageActive, functionName } = currentMenu;

  const onCancel = () => {
    dispatch({
      type: 'configurationExcel/hideExcelModal',
    });
  };
  ...
  const onOk = async () => {
    ...
    const response: any = await dispatch({
      type: `configurationExcel/${dataImageActive ? 'importDataByVersion' : 'importData'}`,
      payload: {
        fileInfoVO,
        functionId,
        totalRecords,
    ...
    if (response.success) {
      ...
      dispatch({
        type: 'configurationExcel/resetExcelData',
      });
      ...
      if (response.type === 'validation_exception') {
        const errors = getErrorForResponseImport(response.promptMessages[0].code);

        dispatch({
          type: 'configurationExcel/saveImportErrors',
          payload: {
            importErrors: errors,
          },
        });
  ...
  return (
    ...
    >
      ...
      {importErrors && importRowData.length > 0 && (
        <>
          <Divider dashed />
          ...
          <Table
            rowKey={(r, i) => `${i}`}
            columns={importErrors.columns}
            scroll={{ x: 'max-content' }}
            dataSource={importRowData || []}
          />
        </>
      )}
