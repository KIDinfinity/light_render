...
function WarnModal(props: any) {
  ...
  const showWarnModal = useSelector(
    (state: any) => state.dataConfigurationController?.showWarnModal
  );
  const isAdd = useSelector((state: any) => state.dataConfigurationController?.isAdd);
  const { form } = props;
  const expiryDate = form.getFieldValue('expiryDate');
  const effectiveDate = form.getFieldValue('effectiveDate');
  const onCancel = () => {
    dispatch({
      type: 'dataConfigurationController/hideWarnModal',
    });
    dispatch({
...
export default connect(({ dataConfigurationController }: any) => ({
  headerData: dataConfigurationController?.headerData,
}))(
  Form.create({
    onFieldsChange(props: any, changedFields: any) {
      const { dispatch } = props;
      dispatch({
        type: 'dataConfigurationController/saveHeaderData',
        payload: {
          changedFields,
        },
      });
