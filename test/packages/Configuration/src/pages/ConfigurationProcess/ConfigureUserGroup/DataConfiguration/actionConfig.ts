...
import { getIsUnSaveForm, getObjectData } from 'configuration/utils';
...
const getSubmitRecords = (changeData: any) => {
  const handleChangeData = lodash.map(changeData, (data: any) => {
    ...
    return {
      ...
      subSection: lodash.map(item?.subSection, (sectionItem: any) => {
        return {
          ...sectionItem,
          data: {
            ...sectionItem?.data,
...
const submitCallBack = (dispatch: any, confirm: any, isUnSaveForm: any) => {
  ...
  tenant.region({
    ...
    notMatch() {
      if (confirm) return;
      dispatch({
        type: 'configureUserGroupController/showWarnModal',
      });
    },
  });
};
...
export default {
  submit: {
    validate: {
      ...
      progress: async ({ dispatch }: any) => {
        const claimProcessData = await dispatch({
          type: 'configureUserGroupController/getClaimProcessData',
        });
        const errors = await dispatch({
          type: 'configureUserGroupController/validateFields',
        });
    ...
    anyway: ({ dispatch, responseCollect }: any) => {
      ...
      if (
        ...
        dispatch({
          type: 'configureUserGroupController/findDuplicateData',
          payload: {
  ...
  upload: {
    ...
    onChange: async ({ files, dispatch }: any) => {
      await dispatch({
        type: 'configureUserGroupController/importExcel',
  ...
  template: {
    ...
    after: async ({ claimStates, dispatch }: any) => {
      ...
      const response: any = await dispatch({
        type: 'configureUserGroupController/exportTemplate',
        payload: {
          functionId,
        },
      });
