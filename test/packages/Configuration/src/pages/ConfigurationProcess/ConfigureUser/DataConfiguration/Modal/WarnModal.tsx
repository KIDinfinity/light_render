...
function WarnModal(props: any) {
  ...
  const isAdd = useSelector((state: any) => state.configureUserController?.isAdd);
  const { form } = props;
  const expiryDate = form.getFieldValue('expiryDate');
  const effectiveDate = form.getFieldValue('effectiveDate');
  const onCancel = () => {
    dispatch({
      type: 'configureUserController/hideWarnModal',
    });
    dispatch({
...
export default connect(({ configureUserController }: any) => ({
  headerData: configureUserController?.headerData,
}))(
  Form.create({
    onFieldsChange(props: any, changedFields: any) {
      const { dispatch } = props;
      dispatch({
        type: 'configureUserController/saveHeaderData',
        payload: {
          changedFields,
        },
      });
