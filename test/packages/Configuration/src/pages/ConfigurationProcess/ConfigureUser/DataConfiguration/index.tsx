...
function DataConfiguration(props: any) {
  ...
  const functionLoading = useSelector(
    (state: any) => state.loading.effects['configureUserController/getClaim']
  );
  const isUpdate = useSelector((state: any) => state.configureUserController.isUpdate);
  const mode = useSelector((state: any) => state.configureUserController?.mode);
  const functionCode = useSelector(
    (state: any) => state.configureUserController?.functionData?.functionCode
  );
  const functionName = useSelector(
    (state: any) => state.configureUserController?.functionData?.functionName
  );
  const pageTemplateType = useSelector(
    (state: any) => state.configureUserController?.functionData?.task?.pageTemplateType
  );
  const showWarnModal = useSelector((state: any) => state.configureUserController.showWarnModal);
  const { previewRecord, taskDetail } = props;
  const clsString = classNames(styles.main, { [styles.isNoPadding]: Mode.Abbreviated === mode });
  useEffect(() => {
    dispatch({
      type: 'configureUserController/getClaim',
      payload: {
        previewRecord,
    ...
    return () => {
      dispatch({
        type: 'configureUserController/clearClaim',
      });
    };
  }, []);
  return (
