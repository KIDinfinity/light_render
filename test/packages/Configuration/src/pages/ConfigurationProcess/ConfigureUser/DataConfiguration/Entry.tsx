...
export default ({ taskDetail, previewRecord }: any) => {
  ...
  const claimProcessData = useSelector((state: any) => state.configureUserController.formData);
  const mode = useSelector((state: any) => state.configurationController.mode);
  const errors = useMemo(() => {
    return formUtils.getErrorArray(claimProcessData);
  }, [claimProcessData]);
  bpm.setClaimDataSelector((state: any) => state.configureUserController);
              ...
              <BPM.HeaderInfoItem
                renderValue={() => (
                  <div className={classnames(styles.container, styles.Abbreviated)}>
                    <Arrow mode={mode} type='configureUserController' />
                  </div>
                )}
              />
            ) : (
