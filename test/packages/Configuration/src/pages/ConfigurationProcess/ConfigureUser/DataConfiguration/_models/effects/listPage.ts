...
export default [
  function* ({ payload = {} }: any, { put, call, select }: any) {
    ...
    const {
      ...
      functionData: state.configureUserController?.functionData,
    ...
    if (response?.success) {
      const list = companyCodeHandler.toFE(response.resultData.rows);
      if (isUpdate) {
        const record = list?.[0];
        yield put({
          type: 'saveFormData',
          ...
          (newList, item): any => {
            ...
            lodash.forEach(subSectionList, (Fitem: any) => {
              const exist = lodash.some(
                newList,
                (nItem) => nItem?.data?.group_code === Fitem?.data?.group_code
              );
              if (!exist) {
