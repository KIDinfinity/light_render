...
class ExcelUtils {
  ...
  handleExport = (
    ...
    workSheet.columns = this.getExcelHeaderByData(DataRows[0], structure, true, false);
    workSheet.addRows(DataRows);
    workSheet.columns = this.getExcelHeaderByData(DataRows[0], structure, true, true);

    this.setOperaBtnOfList(workSheet, recordOperationType);
    this.setWhereCriteriaOfCell(workSheet, structure, true);
    ...
    structureSheet.columns = this.getExcelHeaderByData(structureData[0], structure, false, true);
  ...
  handleExportFormat = (
    ...
    const workSheet = workbook.addWorksheet('dataList');
    const DataRows = this.transferRowData(dataList, structure, recordOperationType[0]);
    const columns = this.getExcelHeaderByDataFormat(DataRows[0], structure, true).filter(
      (item) => item.header !== '#operation'
    );
    workSheet.columns = this.getExcelHeaderByData(DataRows[0], structure, true, false).filter(
      (item) => item.header !== '#operation'
    );
    workSheet.addRows(DataRows);
  ...
  handleExportTemplate = (
    ...
    const structureSheet = workbook.addWorksheet('structure');
    const structureData = this.getSheetDataField(structure);
    structureSheet.columns = this.getExcelHeaderByData(structureData[0], structure, false, false);
    structureSheet.addRows(structureData);
    structureSheet.columns = this.getExcelHeaderByData(structureData[0], structure, false, true);
    this.setHeadAutoSize(structureSheet);
  ...
  getExcelHeaderByData = (
    ...
    return lodash.map(lodash.keys(data), (el) => {
      const { componentType = '', visible } =
        lodash.find(dataFieldList, (item) => item.fieldName === el) || {};
      const formatKey = o ? `${el}/${getFormat(el)}` : el;
      const temp = {
        header: noFormatName.includes(el) ? el : formatKey,
        key: noFormatName.includes(el) ? el : formatKey,
        hidden: !visible,
      };
  ...
  // 处理主键列颜色
  setWhereCriteriaOfCell = (workSheet: Worksheet, structure: DataFieldProps[], o): void => {
    lodash
      .chain(structure)
      .filter((item: any) => item.whereCriteria)
      .map((el: any) => {
        const formatKey = o ? `${el.fieldName}/${getFormat(el.fieldName)}` : el.fieldName;

        const keyRow = workSheet.getColumn(noFormatName.includes(el) ? el.fieldName : formatKey);
  ...
  getImportExcelData = (sheet: Worksheet, dataField: DataFieldProps[] = []) => {
    ...
    sheet.eachRow((row: any, rowNumber: number) => {
      ...
      if (rowNumber === 1) {
        header = values.map((item) => item.split('/')?.[0]);
      } else {
    ...
    header = header.filter((item) =>
      dataField.map((dataItem) => dataItem.fieldName).includes(item)
    );
    ...
    const columns = lodash.map(header, (item: any) => {
      return {
        title: noFormatName.includes(item) ? item : `${item}/${getFormat(item)}`,
        dataIndex: item,
        key: item,
        render: (text: any) => this.renderTagsByExcelType(item, text, dataField),
      };
    });
    return {
    ...
    sheet.eachRow((row): void => {
      row.eachCell((cell, cellIdx: number): void => {
        if (typeof cell.value === 'string') {
          ...
          MaxColumns[cellIdx] = Math.max(
            MaxColumns[cellIdx] || 0,
            cellWidth > 1000 ? 1000 : cellWidth
          );
        }
      });
    });
