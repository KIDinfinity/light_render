...
function ExportTemplate(props: ComponentProps) {
  ...
  const exportTemplateLoading: boolean = useSelector(
    (state: any) => state.loading.effects['configurationController/exportTemplate']
  );
  const {
  ...
  const handlerExportTemplate = async () => {
    const response: any = await dispatch({
      type: 'configurationController/exportTemplate',
      payload: {
        functionId,
      },
    });
