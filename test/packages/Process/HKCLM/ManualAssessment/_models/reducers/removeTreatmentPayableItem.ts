...
/**
 * HKCLM_ManulAssessment显示的Treatment是C类型的，没有InvoicePayable与ServicePayable
 * @param state
 * @param action
 * @returns
 */
const removeTreatmentPayableItem = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    const policyNo = draftState.claimEntities.treatmentPayableListMap[id]?.policyNo;
    ...
    if (
      ...
      updateClaimPolicyPayableFun.updateNCDFlag(draftState, { policyNo }, { isRemove: true });
    }
  });
