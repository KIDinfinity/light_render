...
const mapSearchName = {
  ...
  deductibleAmount: 'systemDeductibleAmount',
  deductibleWaived: 'systemDeductibleWaived',
...
const benefitItemRecover = (state: any, { payload }: any) => {
  const nextState = produce(state, (draftState) => {
    ...
    const handle = (id: string) => {
      const extra = {
        [fieldName]:
          draftState.claimEntities?.[mapToPayableListMap?.[benefitCategory]]?.[id]?.[mapSearchName?.[fieldName]],
      };
