      ...
      return adjuctmentFactorItem
        ? [
            ...
            ...lodash
              ...
              .map(({ factorCode, isSelected, list = [] }: any) => {
                const displayByPolicyYear =
                  lodash
                    .chain(listBenefitFactor)
                    .find({ factorCode })
                    .get('displayByPolicyYear')
                    .value() || false;
                return {
                  ...claimPayableItem,
                  factorCode,
                  isSelected,
                  factorValue: !displayByPolicyYear
                    ? list[0]?.factorValue
                    : lodash.chain(list).find({ policyYear }).get('factorValue').value(),
