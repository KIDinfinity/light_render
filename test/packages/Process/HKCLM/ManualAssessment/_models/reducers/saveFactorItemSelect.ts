...
const saveFactorItemSelect = (state: any, { payload }: any) => {
  return produce(state, (draftState: any) => {
    ...
    // originData为isSelect 原始值
    if (lodash.size(changedFields) === 1 && factorItem.originData === undefined) {
      factorItem.originData = factorItem.isSelected === undefined ? 0 : factorItem.isSelected
    }
