...
export default function* validateFields(_, { select, put }: any) {
  ...
  const claimProcessData = yield select((state) => state.JPCLMOfDataCapture.claimProcessData);
  const claimEntities = yield select((state) => state.JPCLMOfDataCapture.claimEntities);
