...
@connect(({ JPCLMProcessCreate }: any, { treatmentId }: any) => ({
  claimNo: lodash.get(JPCLMProcessCreate, 'claimProcessData.claimNo'),
  procedureList: getProcedureList(treatmentId, JPCLMProcessCreate.claimEntities.procedureListMap),
}))
class ProcedureList extends Component<IProps> {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'JPCLMProcessCreate/addProcedureItem',
      payload: {
        treatmentId,
        addProcedureItem,
      },
