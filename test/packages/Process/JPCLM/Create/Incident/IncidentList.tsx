...
@connect(({ JPCLMProcessCreate, formCommonController, claimEditable }: any) => ({
  incidentList: lodash.get(JPCLMProcessCreate, 'claimProcessData.incidentList'),
  claimNo: lodash.get(JPCLMProcessCreate, 'claimProcessData.claimNo'),
...
class IncidentList extends PureComponent {
  handleAdd = () => {
    ...
    dispatch({
      type: 'JPCLMProcessCreate/addIncidentItem',
      payload: {
        addIncidentItem,
        addDiagnosisItem,
      },
  ...
  render() {
    const { incidentList, submited }: any = this.props;
