...
@connect(({ JPCLMProcessCreate, formCommonController }: any, { invoiceId }: any) => ({
  ...
  claimNo: JPCLMProcessCreate.claimProcessData.claimNo,
  serviceItemList: JPCLMProcessCreate.claimEntities.invoiceListMap[invoiceId].serviceItemList,
  serviceItemListMap: JPCLMProcessCreate.claimEntities.serviceItemListMap,
}))
class ServiceItemList extends Component<IProps> {
  handleAdd = () => {
    ...
    dispatch({
      type: 'JPCLMProcessCreate/addServiceItem',
      payload: {
        invoiceId,
        addServiceItem,
      },
