...
@connect(({ JPCLMProcessCreate, user }) => ({
  claimProcessData: JPCLMProcessCreate.claimProcessData,
  payeeList: lodash.get(JPCLMProcessCreate, 'claimProcessData.payeeList', [])[0],
  user,
}))
class Create extends Component {
  ...
  getDropdown = () => {
    ...
    dispatch({
      ...
      payload: dictionaryConfig?.JPCreate,
    });
    dispatch({
  ...
  handleSubmit = async () => {
    ...
    const errors = await dispatch({
      type: 'JPCLMProcessCreate/validateFields',
    });
    ...
    const response = await dispatch({
      type: 'JPCLMProcessCreate/create',
    ...
    if (response?.success) {
      ...
      const result = await dispatch({
        type: 'JPCLMProcessCreate/getClaimCaseNo',
        payload: {
          claimNo: businessNo,
        },
      });
