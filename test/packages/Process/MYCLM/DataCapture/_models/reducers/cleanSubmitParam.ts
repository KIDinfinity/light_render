...
const cleanSubmitParam = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (
      formUtils.queryValue(draftState.claimProcessData.claimant.relationshipWithInsured) !== 'T'
    ) {
      draftState.claimProcessData.claimant = {};
    }
    // Payee type = others 时，不用删掉 payee
    const newPayeeListMap = lodash.reduce(
      lodash.values(draftState.claimEntities.payeeListMap),
      (res, i) => {
