...
export const FormItem = ({ isShow, layout, form, editable, field, config, incidentId }: any) => {
  ...
  const { serviceItemListMap, existedClaimTypes, currentClaimType } = useSelector(
    ({ [NAMESPACE]: modelnamepsace }: any) => {
      ...
      return {
        ...
        existedClaimTypes: lodash.compact(
  ...
  const currentClaimOption = dictsOfClaimType?.find(
    (option) => option.dictCode === currentClaimType
  );
  ...
  const filteredClaimTypes = !existedClaimTypes.length
    ? dictsOfClaimType
    : existedClaimTypes.includes('DTH') && currentClaimType !== 'DTH'
    ? []
    : dictsOfClaimType?.filter(
        (option) =>
          !existedClaimTypes.includes(option.dictCode) &&
          ((existedClaimTypes.length === 1 && existedClaimTypes[0] === currentClaimType) ||
            option.dictCode !== 'DTH')
      ) || [];
