...
const ButtonGroup = ({ incidentId, treatmentId, invoiceId, serviceItemId }: any) => {
  ...
  const icon = !useMemo(
    () =>
      lodash.some(claimServiceItemBreakDownList, (item) => {
        return (
          lodash.isNumber(formUtils.queryValue(item.expense)) ||
          lodash.isNumber(formUtils.queryValue(item.unit))
        );
      }),
    [claimServiceItemBreakDownList]
  );
  ...
  return (
    <div className={styles.btnGroup}>
      {requireBreakdown === BreakDown.YES && (
        <Button className={styles.btn} onClick={showBreakdown}>
