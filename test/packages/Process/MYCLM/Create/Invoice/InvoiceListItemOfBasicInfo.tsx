...
@connect(({ hkProcessController, formCommonController }: any, { invoiceId }: any) => {
  const invoiceItem = hkProcessController.claimEntities.invoiceListMap[invoiceId];
  const treatmentItem =
    hkProcessController.claimEntities.treatmentListMap[invoiceItem?.treatmentId];
  return {
    invoiceItem,
    validating: formCommonController.validating,
    incidentItem: hkProcessController.claimEntities.incidentListMap[treatmentItem?.incidentId],
  };
})
// @ts-ignore
@Form.create({
  onFieldsChange(props: IProps, changedFields) {
    ...
    if (formUtils.shouldUpdateState(changedFields)) {
      dispatch({
        type: 'hkProcessController/saveInvoiceItem',
        payload: {
          changedFields,
          invoiceId,
        },
