...
export const FormItem = ({
  ...
  const assessmentDecision = useSelector(
    ({ [NAMESPACE]: modelnamepsace }: any) =>
      lodash.get(modelnamepsace.claimProcessData, 'claimDecision.assessmentDecision') || ''
  );
  ...
  const visibleConditions = Rule(fieldProps['editable-condition'], form, '');
  const editableConditions = !Rule(fieldProps['editable-condition'], form, '');
  const requiredConditions = Rule(fieldProps['required-condition'], form, '');
  const Rules = {
    VLD_000271: Validator.VLD_000271(formUtils.queryValue(assessmentDecision)),
  };
  return (
    isShow &&
