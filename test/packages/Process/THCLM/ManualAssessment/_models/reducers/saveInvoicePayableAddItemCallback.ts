...
const saveInvoicePayableAddItemCallback = (state: any) => {
  const { claimEntities, invoicePayableAddItem, exchangeRateListForInvoiceCurrencyPayout } = state;
  ...
  if (treatmentPayableArray && treatmentPayableArray.length === 1) {
    ...
    if (!curPayableIsAlreadyExist) {
      ...
      if (
        incidentPayableArray[0].benefitCategory === BenefitCategory.reimbursement &&
      ...
      // 添加ExchangeRateRecord
      const fromCurrency = invoiceItem?.invoiceCurrency || systemCurrency;
      const toCurrency =
        newClaimEntities.claimPayableListMap[incidentId]?.policyCurrency || systemCurrency;
      const exchangeRate = getExchangeRateValue({
        exchangeRateList: exchangeRateListForInvoiceCurrencyPayout,
        toCurrency,
        fromCurrency,
      });
      newInvoicePayableAddItem.exchangeRateInvoicePolicy = exchangeRate;
      newInvoicePayableAddItem.exchangeRateRecord = `${fromCurrency}/${toCurrency} = ${lodash
        .toNumber(exchangeRate)
        .toFixed(6)} (${moment(new Date()).format('L')})`;
      newClaimEntities.invoicePayableListMap[newInvoicePayableAddItem.id] =
        newInvoicePayableAddItem;
