...
const localConfig = {
  configs: [localSectionConfig, ...BasicFieldConfigs, ...HeaderFieldConfigs, ...CheckFieldConfigs],
  remote: true, // 远程配置来源于本地配置，当远程配置同步后，改remote为true
};

const SectionLayout = ({ form, config, layoutName, children }: any) => (
  <FixedFieldLayout form={form} config={config} layoutName={layoutName}>
    {children}
  </FixedFieldLayout>
);
    ...
    <div>
      <FormRegister form={form} register={register}>
        <Form layout="vertical">
          <SectionLayout form={form} layoutName={layoutName} config={config}>
            {children}
          </SectionLayout>
        </Form>
      </FormRegister>
...
export { HeaderFields, CheckFields, BasicFields, SectionTitle, localConfig };
