/* eslint-disable no-param-reassign */
import { produce }  from 'immer';
import lodash from 'lodash';
import {v4 as uuidv4 } from 'uuid';
import { formatMessageApi } from '@/utils/dictFormatMessage';
import DecisionEnum from '../../../common/DecisionEnum';

export default (state: any, { payload }: any) =>
  produce(state, (draftState: any) => {
    const { id } = payload;
    const cloneOriginSortList: any[] = [];

    // 初始化时获取克隆源数据并且排序

    const cloneOriginListMap = lodash.reduce(
      draftState?.processData?.policyInfo?.applyToPolicyInfoList,
      (result, value) => {
        const newId = uuidv4();

        cloneOriginSortList.push(`${id}____${value.policyId}`);
        return {
          ...result,
          [`${id}____${value.policyId}`]: {
            ...value,
            ...draftState.entities.policyInfoBOListMap?.[`${id}____${value.policyId}`],
            id: newId,
          },
        };
      },
      {}
    );

    draftState.sortApplyListMap[id] = cloneOriginSortList
      .sort((a: any) =>
        lodash.includes(draftState.entities.transactionTypesMap?.[id]?.applyToPolicyBOList, a)
          ? -1
          : 0
      )
      .sort((a: any) => (a.split('____')[1] === draftState.processData?.mainPolicyId ? -1 : 0));

    draftState.entities.policyInfoBOListMap = {
      ...draftState.entities.policyInfoBOListMap,
      ...cloneOriginListMap,
    };

    // 不存在时默认选中TBC
    if (
      !/complete/i.test(draftState.processData.taskStatus) &&
      !draftState.entities.transactionTypesMap[id].decision
    ) {
      draftState.entities.transactionTypesMap[id].decision = {
        dirty: false,
        label: 'Decision',
        name: 'decision',
        validating: false,
        value: DecisionEnum.TBC,
        errors: [
          {
            field: 'decision',
            message: formatMessageApi({ Label_COM_Message: 'MSG_000550' }),
          },
        ],
      };
    }

    const mapMainPolicy =
      draftState.entities.policyInfoBOListMap[`${id}____${draftState?.processData?.mainPolicyId}`];
    if (mapMainPolicy && !mapMainPolicy.policyDecision) {
      mapMainPolicy.policyDecision = draftState.entities.transactionTypesMap[id].decision;
    }
  });
