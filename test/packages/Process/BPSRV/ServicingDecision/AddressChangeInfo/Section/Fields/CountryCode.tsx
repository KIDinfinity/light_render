...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const countries =
    useSelector(({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.address?.country) || [];

  const dicts = countries.map((country: any) => ({
    dictCode: country.code,
    dictName:
      tenant.region() !== Region.ID
        ? formatMessageApi({ Dropdown_CFG_Country: country.code })
        : country.description,
  }));
...
const CountryCode = ({ isShow, layout, form, editable, section }: any) => (
  <Authority>
