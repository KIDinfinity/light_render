...
const getClientName = ({ organization, companyName, firstName, middleName, surname }: any) => {
  return !!organization
    ? companyName
    : lodash.compact([firstName, middleName, surname].map((data) => data)).join(' ');
};
...
export const FormItem = ({
  ...
  const payeeList = useSelector(({ [NAMESPACE]: modelnamespace }: any) =>
    formUtils.cleanValidateData(modelnamespace?.paymentModal?.datas?.payeeList)
  );
  const payeeNames =
    payeeList?.filter(({ payeeType }: any) => payeeType === payToType)?.map(getClientName) || [];
    ...
    ((config?.visible || fieldProps.visible) === Visible.Conditions
      ...
      <Col {...layout}>
        <FormItemAutoComplete
          ...
          onBlur={(fullName) => {
            dispatch({
              type: `${NAMESPACE}/paymentPayeeAllocationLinkPayee`,
              payload: {
                benefitItemId: benefitItem.id,
                id,
                fullName,
                isOrganization: !!currentPayee?.organization,
              },
            });
          }}
          disabled={
