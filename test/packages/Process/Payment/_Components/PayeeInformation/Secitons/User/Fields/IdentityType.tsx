...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const { OrganizationIdentityType = [], IdentityType = [] } =
    useSelector(({ dictionaryController }: any) => dictionaryController) || {};

  const visibleConditions = !isOrganization;
  ...
  return (
    ...
    ((config?.visible || fieldProps.visible) === Visible.Conditions
      ...
      <Col {...layout}>
        <FormItemSelect
          dicts={isOrganization ? OrganizationIdentityType : IdentityType}
          disabled={
            !editable ||
