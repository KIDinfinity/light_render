...
const PaymentRow = ({ children, className }: any) => {
  return (
...
export default ({ NAMESPACE }: any) => {
  const {
    ...
    policyBenefitList = [],
  } = useSelector(({ [NAMESPACE]: modelnamespace }: any) => modelnamespace?.paymentModal?.datas) ||
  {};
  ...
  const getPaymentRow = ({ id: payeeId, payoutCurrency }: any) => {
    ...
    return list.map(({ policyNo, beneficiaryAmount }: any) => (
      <PaymentRow key={policyNo} className={styles.solidWhite}>
        {policyNo || ''}
        {`${formUtils.queryValue(beneficiaryAmount)?.toFixed(2) || 0} ${
          formUtils.queryValue(payoutCurrency) || ''
        }`}
      </PaymentRow>
    ));
  };
          ...
          <div className={styles.payeeCard} key={payee.id}>
            ...
            <div className={styles.payeeRight}>
              <PaymentRow key={'title'}>
                <PayoutCurrency item={payee} NAMESPACE={NAMESPACE} />
              ...
              <div className={styles.paymentMethod}>
                ...
                >
                  <PaymentMethod
                    payee={payee}
                    totalPayoutAmount={getTotalPayoutAmount(payee)}
                    NAMESPACE={NAMESPACE}
                  />
                </SectionCard>
              </div>
