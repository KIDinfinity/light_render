...
const removeInvoiceItem = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState) => {
    ...
    const invoiceDate = formUtils.queryValue(draftState?.claimEntities?.invoiceListMap[invoiceId]?.invoiceDate);
    ...
    lodash.map(treatmentListMap, (treatment: any) => {
      const invoiceListMapDataN = formUtils.cleanValidateData(draftState.claimEntities.invoiceListMap);
      ...
      lodash.map(newInvoiceList, (item: any, key) => {
        ...
        draftState.claimEntities.invoiceListMap[item.id].invoiceNo = getInvoiceNo(invoiceNumber, item.invoiceDate);
      });
    });
  });
