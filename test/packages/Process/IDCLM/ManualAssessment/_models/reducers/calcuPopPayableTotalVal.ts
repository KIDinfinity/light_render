...
const calcuPopPayableTotalVal = (state: any, { payload }: any) => {
  ...
  return produce(state, (draftState: any) => {
    ...
    const payableAmount = !data?.isUpdate ? getPayableAmount(data) : data?.payableAmount;

    draftState.popUpPayable.benefitListMap[benefitItemId].listMap[serviceItemId] = {
      ...data,
      chooise,
      payableAmount,
      calculationAmount: payableAmount,
      uncoverAmount: 0,
    };
    ...
    draftState.popUpPayable.benefitListMap[benefitItemId].calculationAmount =
      totalPayableAmount === 0 ? null : totalPayableAmount;
    draftState.popUpPayable.benefitListMap[benefitItemId].uncoverAmount = 0;
  });
};
