...
const updateAmount = ({
  ...
  draftState.claimEntities.serviceItemListMap[serviceItemId][sourceField] = value;
  let totalAmount = 0;
  lodash.map(draftState.claimEntities.invoiceListMap[invoiceId].serviceItemList, (serviceId) => {
    const targetValue = draftState.claimEntities.serviceItemListMap[serviceId][sourceField];
  ...
  produce(state, (draftState: any) => {
    ...
    if (lodash.size(changedFields) === 1) {
      if (lodash.has(changedFields, 'expense')) {
        updateAmount({
          sourceField: 'expense',
          targetField: 'summaryOfServiceItem',
      ...
      if (lodash.has(changedFields, 'otherInsurerPaidAmount')) {
        const otherInsurerPaidAmountAmount = updateAmount({
          sourceField: 'otherInsurerPaidAmount',
