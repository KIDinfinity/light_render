...
const savePayee = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (!lodash.has(changedFields, 'paymentMethod') && isEditStatus) {
      if (
        ((lodash.has(changedFields, 'identityType') &&
          changedFields?.identityType?.value !== 'I') ||
        ...
        if (lodash.isPlainObject(payeeTemp?.paymentMethod)) {
          payeeTemp.paymentMethod.errors = lodash.isArray(payeeTemp?.paymentMethod?.errors)
            ? payeeTemp.paymentMethod.errors.push(MSG_000538)
            : [MSG_000538];
        } else {
