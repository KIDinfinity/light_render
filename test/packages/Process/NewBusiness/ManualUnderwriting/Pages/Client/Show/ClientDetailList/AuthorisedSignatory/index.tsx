...
export default ({ clientId }: any) => {
  ...
  const isEntity = useGetIsCustomerEntity({ customerType });
  const isAuthorisedSignatoryDispay =
    lodash.find(roleDicts, { dictCode: CustomerRole.AuthorisedSignatory })?.display ===
    BooleanEnum.Yes;
  const hasOwner = lodash.includes(customerRole, CustomerRole.PolicyOwner);
  const isShow = isAuthorisedSignatoryDispay && isEntity && hasOwner;
