...
const tableColumns = [
  ...
  },
  {
    ...
    render: (text: string, item: any) =>
      moment(lodash.get(item, 'insuredClientInfo.dateOfBirth')).format('MM/DD/YYYY') || '-',
  },
  {
    title: formatMessageApi({
  ...
  {
    title: formatMessageApi({
      Label_BIZ_Claim: 'dependentNo',
    }),
    dataIndex: 'dependentNo',
    render: (text: string, item: any) => lodash.get(item, 'dependentNo') || '-',
  },
];
...
const SelectInsuredForm = () => {
  ...
  const handleSelectInsured = async () => {
    ...
    dispatch({
      type: `${NAMESPACE}/saveBusinessProcess`,
      payload: {
        selectColumns,
      },
    });
    await dispatch({
      ...
      payload: {
        ...
        insuredInfo: {
          ...
          dependentNo: selectColumns?.dependentNo,
  ...
  const getRowKey = ({ policyId, insuredClientInfo, memberNo }: any) =>
    `${policyId}_${insuredClientInfo?.clientId}_${memberNo} ` || '';
