...
const mapStateToProps = ({ hkProcessController, formCommonController }, { incidentId }) => {
  const { claimEntities } = hkProcessController;
...
class DiagnosisList extends Component<IProps> {
  handleAdd = () => {
    ...
    dispatch({
      type: 'hkProcessController/addDiagnosisItem',
      payload: {
        incidentId,
        addDiagnosisItem,
      },
    ...
    return (
      <div className={styles.diagnosiCard}>
        <div>
          ...
          {lodash.compact(diagnosisList).map((item: any, index: number) => (
            <DiagnosisListItem
