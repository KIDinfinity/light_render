...
@connect(({ hkProcessController }: any, { treatmentId }: any) => ({
  claimNo: lodash.get(hkProcessController, 'claimProcessData.claimNo'),
  procedureList: getProcedureList(treatmentId, hkProcessController.claimEntities.procedureListMap),
}))
class ProcedureList extends Component<IProps> {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'hkProcessController/addProcedureItem',
      payload: {
        treatmentId,
        addProcedureItem,
      },
    ...
    return (
      <div className={styles.procedureCard}>
        <div>
          ...
          {lodash.compact(procedureList).map((item: any, index: number) => (
            <ProcedureItem
