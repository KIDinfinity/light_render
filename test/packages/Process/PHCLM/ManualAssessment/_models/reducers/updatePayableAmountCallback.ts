...
const updatePayableAmountCallback = (state: any) => {
  const nextState = produce(state, (draftState) => {
    const isAppeal = draftState.claimProcessData.appeal;
    const claimEntities = isAppeal? {
        ...draftState.claimEntities,
        claimPayableListMap: lodash.pickBy(draftState.claimEntities.claimPayableListMap, claimPayable => claimPayable.isNewPayable || claimPayable.isPayableAdjusted)
      } : draftState.claimEntities;

    const claimPayableList = isAppeal? draftState.claimProcessData.claimPayableList?.filter(claimPayableId => claimEntities.claimPayableListMap?.[claimPayableId]) : draftState.claimProcessData.claimPayableList;
    if (lodash.isArray(claimPayableList) && claimPayableList.length > 0) {
      // payableAmount逐级计算
      calculatPayableAmount(draftState.claimProcessData, claimEntities, claimPayableList);
      // 更新保单受取人
      ...
      calculatPaymentAmount(draftState.claimProcessData, claimEntities);
      calculatPayable({ claimEntities });
