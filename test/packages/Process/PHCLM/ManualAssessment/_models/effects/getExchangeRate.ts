...
export default function* getExchangeRate({ payload }: any, { put, select, call }: any) {
  ...
  const params = {
    lookupDateList: dateList.filter(date => {
  ...
  if(!params.lookupDateList.length)
    return false;
  ...
  if (success && resultData) {
    const exchangeRate = resultData.map((item: any) => {
      return lodash
        .chain(item)
        .pick(['fromCurrency', 'toCurrency', 'exchangeRate', 'exchangeType', 'effectiveDate']).value()
    });
    const currentExchangeRate = yield select(({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.exchangeRate);
