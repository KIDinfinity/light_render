...
export default function* getExchangeRateForExchangeDate(
  ...
  const invoiceListMap = yield select(
    ({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.claimEntities.invoiceListMap
  );
  const invoicePayableListMap = yield select(
    ({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.claimEntities.invoicePayableListMap
  );
