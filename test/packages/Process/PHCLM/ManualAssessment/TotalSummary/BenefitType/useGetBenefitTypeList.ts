...
import { payableDays } from '../../_models/functions/utils';
...
const findBoosterBenefitType = (claimPayableItem: any, claimEntities: any, listPolicy: any) => {
  const serviceItemPayableListMap =
  ...
  const booster = lodash.reduce(
    ...
    (result: any, item: any) => ({
      ...
      payableDays: payableDays(result?.payableDays || '', formUtils.queryValue(item?.payableDays)),
      systemCalculationAmount: add(
        result?.systemCalculationAmount || 0,
        formUtils.queryValue(item?.systemCalculationAmount)
      ),
      systemPayableDays: payableDays(
...
const summaryPayableDays = (target: any) => {
  target.payableDays =
    lodash.reduce(
      target.children,
      (result, child) => payableDays(result, formUtils.queryValue(child?.payableDays)),
      ''
    ) || '';
  target.systemPayableDays =
    lodash.reduce(
      target.children,
      (result, child) => payableDays(result, formUtils.queryValue(child?.systemPayableDays)),
      ''
    ) || '';
};
