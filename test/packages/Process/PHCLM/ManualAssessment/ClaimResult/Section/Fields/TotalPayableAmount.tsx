...
import { Authority, Editable, FormItemCurrency, Required, Visible } from 'basic/components/Form';
...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const currencis = generateCurrencis(listPolicy);
  const currencydicts = lodash
    .chain(getDrowDownList('Dropdown_CFG_Currency'))
    .map((item) => ({
      currencyCode: item.dictCode,
      currencyName: item.dictName,
    }))
    .compact()
    .concat(currencis)
    .uniqBy('currencyCode')
