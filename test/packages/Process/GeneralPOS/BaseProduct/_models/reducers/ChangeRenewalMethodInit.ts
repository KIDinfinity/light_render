...
export default (state: any, { payload }: any) =>
  produce(state, (draftState: any) => {
    ...
    if (
      lodash.isEmpty(
        lodash.get(draftState, `${transactionPath}.paymentInMethodList[0].paymentBankList`, [])
      )
    ) {
      draftState.entities.transactionTypesMap[transactionId].paymentInMethodList = [];
      lodash.set(draftState, `${transactionPath}.paymentInMethodList[0].payinOption`, OptionEnum.D);
      draftState.entities.transactionTypesMap[
        transactionId
      ].paymentInMethodList[0].paymentBankList = [];
    }
  });
