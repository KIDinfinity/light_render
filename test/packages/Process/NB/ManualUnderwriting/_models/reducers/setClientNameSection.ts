...
export default (state: any, action: any) => {
  ...
  const { businessData, planProductConfig } = state;
  const coverageList = lodash.get(businessData, 'policyList[0].coverageList');
  const clientInfoList = lodash.get(businessData, 'clientInfoList', []);
  ...
  const nextState = produce(state, (draftState: any) => {
    lodash.set(
      draftState,
      `businessData.policyList[0].coverageList[${index}].coverageInsuredList[${insuredIndex}]`,
      { ...finalInsuredItem, ...changedFields, insuredSeqNum, coreCode }
    );
  });
  return {
    ...nextState,
