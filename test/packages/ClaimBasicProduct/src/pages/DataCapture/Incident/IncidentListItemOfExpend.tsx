...
@connect(({ bpOfDataCaptureController, claimEditable }: any) => ({
  submited: bpOfDataCaptureController.submited,
...
class IncidentItem extends PureComponent {
  ...
  handleDelete = () => {
    const { dispatch, incidentId, domRef }: any = this.props;
    deleteWarning({
      domRef,
      sectionRef: this.sectionRef,
      sectionID: SectionID.Incident,
    }).then(() => {
      dispatch({
        type: 'bpOfDataCaptureController/removeIncidentItem',
        payload: {
          incidentId,
        },
      });
