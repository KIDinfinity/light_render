...
@connect(({ bpOfClaimAssessmentController, claimEditable }: any, { serviceItemId }: any) => ({
  curServicePayableList: getServicePayableIdList(
    serviceItemId,
    bpOfClaimAssessmentController.claimEntities.serviceItemPayableListMap
  ),
  servicePayableAddItem: bpOfClaimAssessmentController.servicePayableAddItem,
...
class ServiceListItemOfPayableList extends Component {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'bpOfClaimAssessmentController/addServicePayableItem',
      payload: {
        serviceItemId,
        incidentId,
        treatmentId,
      ...
      <div>
        {curServicePayableList &&
          lodash.map(curServicePayableList, (item) => (
            <ServiceListItemOfPayableListItem serviceItemPayableId={item} key={item} />
          ))}
