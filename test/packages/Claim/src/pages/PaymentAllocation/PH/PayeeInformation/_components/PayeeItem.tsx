...
export default ({ clientId }: any) => {
  const { policyOwnerList = [], policyInsuredList = [], policyBeneficiaryList = [] } =
    useSelector(({ paymentAllocation }: any) => paymentAllocation?.claimData?.c360PolicyInfo) || {};
  ...
  return (
    <div className={styles.payeeItemWrap}>
      {lodash.map(roleList, (code, index) => (
        <div className={styles.title} key={code + index}>{formatMessageApi({ Label_BIZ_Policy: code })}</div>
      ))}
    </div>
  );
};
