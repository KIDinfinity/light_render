...
import { validatorPolicyBenefit } from './sectionValidators';
...
export const validatorData = (claimData: any, params: any = {}) => {
  ...
  const { submissionChannel, caseCategory } = taskDetail;

  const isGOPBillingCase =
    formUtils.queryValue(submissionChannel) === SubmissionChannel.GOPBilling &&
    lodash.includes(
      [CaseCategory.HK_CLM_CTG001, CaseCategory.HK_CLM_CTG002],
      formUtils.queryValue(caseCategory)
    );
      ...
      policyBenefitTemp.beneficiaryList = lodash.map(
        ...
        (beneficiary: BeneficiaryModal) => {
          ...
          const validateCurrency = () => {
            ...
            validateBeneficiary.clientId[0].required = isGOPBillingCase ? false : true;
          };
  ...
  claimData.payeeList = lodash.map(payeeList, (payeeItem: PayeeModal) => {
    ...
    validatePayee.surname[0].required = isGOPBillingCase ? !organization : true;
  ...
  console.log(' claimData.payeeList ', claimData.payeeList);
  return claimData;
};
