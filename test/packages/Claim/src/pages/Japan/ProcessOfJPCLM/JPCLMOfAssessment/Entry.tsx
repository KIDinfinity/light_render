...
import { snapshotCheck } from '@/utils/task';
...
const notCheckSnapshotTaskDefKey: string[] = [
...
export default ({ taskDetail, businessData }: any) => {
  if (!lodash.includes(notCheckSnapshotTaskDefKey, taskDetail.taskDefKey)) {
    useEffect(() => {
      snapshotCheck(
        lodash.pick(taskDetail, ['processInstanceId', 'caseCategory', 'initialDataInherit'])
      );
