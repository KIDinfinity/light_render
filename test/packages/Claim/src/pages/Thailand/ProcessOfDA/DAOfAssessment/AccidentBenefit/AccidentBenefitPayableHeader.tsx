...
class AccidentBenefitPayableHeader extends Component<IProps> {
  ...
  get filterBenefitList() {
    return lodash
      .chain(this.dictOfBenefitItemCode)
      .map((item) => {
        const target = lodash.get(item, 'accidentBenefit.availableUse') === AvailableUse.Y;
        const availableUse = lodash.get(item, 'availableUse') === AvailableUse.N;
        if (!target || availableUse) {
  ...
  handleDelete = () => {
    ...
    dispatch({
      type: 'daOfClaimAssessmentController/removeAccidentBenefitPayableItem',
      payload: {
        treatmentPayableId,
  ...
  render() {
    ...
    const payableDays = form.getFieldValue('payableDays') || 0;
    const { limitCode }: any = accidentBenefitPayableItem;

    return (
      <CardOfClaim
        showButton={!taskNotEditable && accidentBenefitPayableItem.isAdd}
        handleClick={() => {
          this.handleDelete();
        }}
        cardStyle={{
        ...
        <Form layout="vertical">
          <FormLayout json={AccidentBenefitPayableItemLayout}>
            <FormItemSelect
              ...
              rules={[
                {
                  validator: VLD_000200(this.dictOfBenefitItemCode, payableDaysList, payableDays),
                },
        ...
        setTimeout(() => {
          dispatch({
            ...
            payload: {
              ...
              id: accidentBenefitId,
              treatmentPayableId,
              sectionName: 'accidentBenefitPayableListMap'
            },
          });
        }, 0);
      } else {
...
export default connect(
  (
    ...
    const { listPolicy, claimEntities } = daOfClaimAssessmentController;
