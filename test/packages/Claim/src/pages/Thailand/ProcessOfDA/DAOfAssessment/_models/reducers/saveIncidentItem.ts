    ...
    if (changedFields.claimType && lodash.size(changedFields) === 1) {
      ...
      if (formUtils.queryValue(changedFields.claimType) === 'IP') {
        lodash.forEach(treatmentList, (item) => {
          draftState.claimEntities.treatmentListMap[item].treatmentType = 'IP';
          draftState.claimEntities.treatmentListMap[item].dateOfConsultation = null;
        });
      }
      if (formUtils.queryValue(changedFields.claimType) === 'OP') {
        lodash.forEach(treatmentList, (item) => {
          draftState.claimEntities.treatmentListMap[item].treatmentType = 'OP';
          ...
          draftState.claimEntities.treatmentListMap[item].icu = 0;
          draftState.claimEntities.treatmentListMap[item].icuFromDate = null;
          draftState.claimEntities.treatmentListMap[item].icuToDate = null;
        });
      }
    }
