...
export default function* validateFields(action, { select, put }: any) {
  ...
  if (!lodash.isEmpty(skipValidateFormIds)) {
    ...
    lastValidateForms = Object.fromEntries(
      Object.entries(forms).filter(
        ([key]) => !skipValidateFormIds.some((prefix) => key.includes(prefix))
      )
    );
  }
  ...
  const claimProcessData = yield select(
    (state) => state.daOfClaimAssessmentController.claimProcessData
  );
  const claimEntities = yield select((state) => state.daOfClaimAssessmentController.claimEntities);
