...
class DiagnosisForm extends Component<IProps> {
  ...
  render() {
    const { form, disabled, idx, diagnosisList, diagnosisType, dispatch } = this.props;
    return (
      <OPDFormSection
        titleText={<>Diagnosis</>}
      ...
      >
        {lodash.map(diagnosisList, (_, diagnosisIdx: number) => (
          // @ts-ignore
          <CardOfClaim
            className="card"
            key={diagnosisIdx}
            showButton={!disabled && diagnosisList.length > 1}
            handleClick={() => this.delDiagnosisItemFn(diagnosisIdx)}
          >
            <Form layout="vertical">
              <FormLayout json={json}>
                <FormItemSelectPlus
                  ...
                  disabled={disabled}
                  formName={`diagnosisList_${diagnosisIdx}_diagnosisCode`}
                  ...
                  allowClear={false}
                  selectCallbackItem
                  onSelectCallback={async ({ item: { dictName } }: any) => {
                    const baseKey = `claimProcessData.invoiceInforData[${idx}].registration.incidentList[0].diagnosisList.${diagnosisIdx}`;
                    await dispatch({
                      type: 'IdentifyHospitalBatchController/changeData',
                      payload: {
                        key: `${baseKey}.diagnosisName`,
                        value: dictName,
                        idx,
                      },
                    });
                    await dispatch({
                      type: 'IdentifyHospitalBatchController/changeData',
                      payload: {
                        key: `${baseKey}.orderNum`,
                        value: Number(diagnosisIdx) + 1,
                        idx,
                      },
                    });
                  }}
                ...
                <FormItemSelect
                  ...
                  disabled={disabled}
                  dicts={diagnosisType}
                  formName={`diagnosisList_${diagnosisIdx}_diagnosisType`}
                ...
                <FormItemCheckbox
                  form={form}
                  disabled={disabled}
                  formName={`diagnosisList_${diagnosisIdx}_criticalIllness`}
