...
const mapStateToProps = (
  { apOfClaimCaseController, formCommonController, claimEditable },
  { incidentId }
) => {
  const { claimEntities } = apOfClaimCaseController;
  const incidentItem = claimEntities.incidentListMap[incidentId];
  const { diagnosisListMap } = claimEntities;

  return {
    submited: formCommonController.submited,
    claimNo: apOfClaimCaseController.claimProcessData.claimNo,
...
class DiagnosisList extends Component<IProps> {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'apOfClaimCaseController/addDiagnosisItem',
      payload: {
        incidentId,
        addDiagnosisItem,
      },
