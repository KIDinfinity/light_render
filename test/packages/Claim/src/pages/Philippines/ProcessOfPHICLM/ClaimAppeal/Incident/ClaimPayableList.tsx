...
@connect(({ PHCLMOfAppealCaseController, claimEditable }: any, { incidentId }: any) => ({
  claimNo: PHCLMOfAppealCaseController.claimProcessData.claimNo,
...
class ClaimPayableList extends Component {
  ...
  componentDidMount = () => {
    const { curIncidentPayableList, taskNotEditable } = this.props;
    if (lodash.isEmpty(curIncidentPayableList) && !taskNotEditable) {
      this.handleAdd();
    }
  };
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'PHCLMOfAppealCaseController/addClaimPayableItem',
...
export default withContextData(ClaimPayableList);
