...
@connect(
  ({ PHCLMOfAppealCaseController, user, formCommonController, dictionaryController }: any) => ({
    ...
    claimAppealOriginalCase: lodash.get(
      PHCLMOfAppealCaseController,
      'claimProcessData.claimAppealOriginalCase'
    ),
  })
)
@changeWorkSpaceHoc
...
class BPOfManualAssessment extends Component<IProps> {
  ...
  componentDidUpdate = async (preProps: any) => {
    const { businessData: preBusinessData } = preProps;
    const { businessData } = this.props;

    if (businessData.id !== preBusinessData.id) {
      await this.getClaimData();
    }
  };
  ...
  getDropdown = () => {
    ...
    dispatch({
      ...
      payload: dictionaryConfig?.PHClaimAppeal,
    });
  ...
  updatePaymentAmount = async (claimData: any) => {
    ...
    const result = await dispatch({
      type: 'PHCLMOfAppealCaseController/updatePayableAmount',
