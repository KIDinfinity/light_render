...
@connect(
  (
    { PHCLMOfAppealCaseController, formCommonController }: any,
    ...
    const tempTreatment = claimEntities.treatmentPayableListMap[treatmentPayableItemId];

    const { claimDecision } = claimEntities.claimPayableListMap[tempTreatment.payableId];
...
class TreatmentPayableListItemOfAI extends Component<IProps> {
  ...
  render() {
    const { policyBackgrounds, form, treatmentPayableItem, claimDecision, withData } = this.props;
    const { taskNotEditable: notEditable } = this.context;
    const policyNoList = this.getPolicyList();
    const is_declined = claimDecision === 'D';

    const { appealNotEditable, caseType, originalCaseCategory }: any = withData || {};
...
export default withContextData(TreatmentPayableListItemOfAI);
