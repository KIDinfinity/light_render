...
@connect(({ PHCLMOfDataCaptureController, claimEditable }: any, { treatmentId }: any) => ({
  claimNo: lodash.get(PHCLMOfDataCaptureController, 'claimProcessData.claimNo'),
  invoiceList: PHCLMOfDataCaptureController.claimEntities.treatmentListMap[treatmentId].invoiceList,
  invoiceListMap: PHCLMOfDataCaptureController.claimEntities.invoiceListMap,
...
class InvoiceList extends PureComponent {
  handleAdd = () => {
    ...
    dispatch({
      type: 'PHCLMOfDataCaptureController/addInvoiceItem',
      payload: {
        treatmentId,
        addInvoiceItem,
        addServiceItem,
