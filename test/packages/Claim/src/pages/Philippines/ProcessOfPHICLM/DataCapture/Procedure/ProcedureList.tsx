...
@connect(({ PHCLMOfDataCaptureController, claimEditable }: any, { treatmentId }: anys) => ({
  claimNo: lodash.get(PHCLMOfDataCaptureController, 'claimProcessData.claimNo'),
  procedureList:
    PHCLMOfDataCaptureController.claimEntities.treatmentListMap[treatmentId].procedureList,
...
class ProcedureList extends PureComponent {
  handleAdd = () => {
    ...
    dispatch({
      type: 'PHCLMOfDataCaptureController/addProcedureItem',
      payload: {
        treatmentId,
        addProcedureItem,
      },
