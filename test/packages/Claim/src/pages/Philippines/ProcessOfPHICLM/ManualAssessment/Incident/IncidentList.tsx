import React, { PureComponent } from 'react';
import { connect } from 'dva';
import lodash from 'lodash';
import{ v4 as  uuidv4 } from 'uuid';
import { Row, Col } from 'antd';
import { formatMessageApi } from '@/utils/dictFormatMessage';
import { INCIDENTITEM, DIAGNOSISITEM } from '@/utils/claimConstant';
import ButtonOfClaim from 'claim/components/ButtonOfClaim';
import IncidentListItem from './IncidentListItem';

@connect(({ PHCLMOfClaimAssessmentController, claimEditable }: any) => ({
  incidentList: lodash.get(PHCLMOfClaimAssessmentController, 'claimProcessData.incidentList'),
  taskNotEditable: claimEditable.taskNotEditable,
}))
class IncidentList extends PureComponent {
  handleAdd = () => {
    const { dispatch, claimNo, incidentList }: any = this.props;
    const incidentId = uuidv4();
    const diagnosisId = uuidv4();
    let incidentNo = 1;
    if (lodash.isArray(incidentList)) {
      incidentNo = incidentList.length + 1;
    }

    const addIncidentItem = {
      ...INCIDENTITEM,
      claimNo,
      diagnosisList: [diagnosisId],
      id: incidentId,
      incidentNo,
    };
    const addDiagnosisItem = {
      ...DIAGNOSISITEM,
      claimNo,
      id: diagnosisId,
      incidentId,
    };

    dispatch({
      type: 'PHCLMOfClaimAssessmentController/addIncidentItem',
      payload: {
        addIncidentItem,
        addDiagnosisItem,
      },
    });
    dispatch({
      type: 'PHCLMOfClaimAssessmentController/setIncidentItemExpandStatus',
      payload: {
        id: incidentId,
        status: true,
      },
    });
  };

  render() {
    const { incidentList, taskNotEditable }: any = this.props;

    return (
      <div>
        {lodash.isArray(incidentList) &&
          incidentList.length > 0 &&
          lodash.map(incidentList, (item) => (
            <IncidentListItem key={item} total={incidentList.length} incidentId={item} />
          ))}
        {!taskNotEditable && incidentList?.length < 1 && (
          <Row type="flex" gutter={16}>
            <Col span={8}>
              <div>
                <ButtonOfClaim
                  handleClick={this.handleAdd}
                  buttonText={formatMessageApi({
                    Label_BPM_Button: 'app.navigator.task-detail-of-data-capture.button',
                  })}
                  buttonStyle={{ width: '100%', height: '36px' }}
                />
              </div>
            </Col>
          </Row>
        )}
      </div>
    );
  }
}

export default IncidentList;
