...
const removeBenefitPayableItem = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draft: any) => {
    ...
    const benefitPayableList = lodash.get(draftState, `claimEntities.invoicePayableListMap.${invoicePayableId}.benefitItemPayableList`);
    draftState.claimEntities.invoicePayableListMap[invoicePayableId].benefitItemPayableList = lodash.filter(benefitPayableList, item => item !== benefitPayableId);
