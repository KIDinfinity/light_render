...
const saveTreatmentPayableAddItemCallback = (state: any) => {
  ...
  if (incidentPayableArray && lodash.size(incidentPayableArray) === 1) {
    ...
    if (
      ...
      invoicePayableList = lodash.map(treatmentItem.invoiceList, (invoiceId) => {
        ...
        if (lodash.isArray(invoiceItem.serviceItemList) && invoiceItem.serviceItemList.length > 0) {
    ...
    lodash.set(
      newClaimEntities,
      `claimPayableListMap.${currentIncidentPayableItem?.id}.treatmentPayableList`,
      [
        ...lodash.get(
          newClaimEntities,
          `claimPayableListMap.${currentIncidentPayableItem?.id}.treatmentPayableList`,
