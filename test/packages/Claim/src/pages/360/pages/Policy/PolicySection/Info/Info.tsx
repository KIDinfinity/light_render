...
export default ({ info, layout = 6 }: any) => {
  ...
  const showPdType = () => {
    ...
    if(pdType) {
      ...
      return <DataItem className={styles.infoItem} title={'P免区分'}>{text}</DataItem>
    }
      ...
      <DataLayout span={layout}>
        ...
        >
          {transactionLogPolicyNotesUrl && (
            <a href={transactionLogPolicyNotesUrl} target="_blank" className={styles.policyNotice} onClick={() => {tarckInquiryPoint(dispatch, {eventName: eEventName.policyNotice, eventOperation: eEventOperation.viewDetail})}} rel="noreferrer">Trans.Log/Policy Notes</a>
          )}
        </DataItem>
        <DataItem
          className={styles.infoItem}
          title={formatMessageApi({ Label_BIZ_Claim: 'Company' })}
        >
          {info?.subCompanyCode}
        </DataItem>
        <DataItem
          className={styles.infoItem}
          title={formatMessageApi({ Label_BIZ_Policy: 'ContractType' })}
        >
          {info?.contractType && `${info.contractType} - ${formatMessageApi({ Dropdown_PRD_ContractType: info.contractType })}`}
        </DataItem>
        <DataItem
          className={styles.infoItem}
          title={formatMessageApi({ Label_COM_General: 'primaryInsuredName' })}
        >
          {info?.insured?.primaryInsuredName}
        </DataItem>
         {
          showPdType()
         }
