...
export default function* changeReminderRoleInfo({ payload }: IAction, { call, select, put }: any) {
  ...
  const { reasonCode, startTime, period, reminderIdx } = lodash.pick(ctn, [
    'reasonCode',
    'startTime',
    'period',
    'reminderIdx',
  ...
  const reasonConfigs = lodash
    ...
    .value();
  const newChannelData = getChannelDataList(
