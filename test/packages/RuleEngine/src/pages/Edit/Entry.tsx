...
export default ({ taskDetail }: any) => {
  const claimProcessData = useSelector(
    (state) => state.ruleEngineController.submitRuleSet,
    shallowEqual
  );
  const businessId = claimProcessData?.ruleSetInfo?.businessId || '';
        ...
        <BPM.HeaderInfoContainer>
          <BPM.HeaderInfo>
            ...
            <BPM.HeaderInfoItem
              key="ruleSetId"
              title={formatMessageApi({
                Label_BIZ_Claim: 'venus_claim.rules.basicInfo.label.setId',
              })}
              value={businessId}
            />
            <BPM.HeaderInfoItem
