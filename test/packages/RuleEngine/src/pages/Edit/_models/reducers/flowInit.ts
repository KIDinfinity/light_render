...
export default (state: any) =>
  produce(state, (draftState: any) => {
    ...
    if (
      ...
      // 新增StartNode
      draftState.submitRuleSet.flowNodeVOs.unshift({
        ruleFlowId: '',
    ...
    decisionNodes.forEach((decisionNode) => {
      ...
      if (!hasAddNode) {
        ...
        const nodes = lodash
          .chain(draftState.submitRuleSet.flowNodeVOs)
          .filter(
            (vo) => vo.nodeIdParent === decisionNode.nodeId && vo.decisionBranchId && vo.nodeType
          )
          .map((vo) => ({
        ...
        draftState.submitRuleSet.flowNodeVOs = lodash.concat(
          [],
          lodash.map(draftState.submitRuleSet.flowNodeVOs, (vo) => {
     ...
     * SpanShot：存EndNode
     */
    if (!lodash.find(draftState.submitRuleSet.flowNodeVOs, (vo) => vo.nodeId === END_NODE_ID)) {
