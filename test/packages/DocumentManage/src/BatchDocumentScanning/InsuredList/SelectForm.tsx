...
const SelectInsuredForm = ({ taskDetail }: any) => {
  ...
  const { insuredSectionIndex, insuredList } = useSelector(
    ({ batchDocumentScanningController }: any) => ({
      insuredList: batchDocumentScanningController.insuredList,
      insuredSectionIndex: batchDocumentScanningController.insuredSectionIndex,
    })
  );
  const policyNo = useSelector(({ batchDocumentScanningController }: any) => {
    const sectionIndex = batchDocumentScanningController.insuredSectionIndex;
    return batchDocumentScanningController?.claimProcessData?.[sectionIndex]?.policyNo || '';
  });
  ...
  const hideShowModal = () => {
    ...
    dispatch({
      ...
      payload: {
        insuredList: [],
        insuredSectionIndex: null,
      },
    });
  };
  const handleSelectInsured = useCallback(async () => {
    ...
    await dispatch({
      type: `batchDocumentScanningController/savePolicyInfo`,
      payload: {
  ...
  }, [policyNo, insuredSectionIndex, selectColumns]);
