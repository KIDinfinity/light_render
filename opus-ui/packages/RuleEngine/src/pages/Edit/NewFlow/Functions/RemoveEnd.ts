    ...
    return {
      flowNodeVOs: lodash
        ...
        .map((flowVo: any) => ({
          ...flowVo,
          isEmpty:
            lodash.find(parentArr, (parentItem: any) => parentItem.nodeIdParent === flowVo.nodeId)
              &&
              flowVo.nodeType !== NodeType.DecisionNode ? true : item.isEmpty
        }))
        .filter((flowVO: any) => flowVO.nodeId !== id && flowVO.nodeId !== nodeIdParent)
        .value() || [],
      branchVOs: lodash.map(branchVOs, (branchVo: any) => ({
        ...branchVo,
        isEmpty: lodash.find(parentArr, (parentItem: any) => parentItem.nodeId === branchVo.nodeId) ? false : branchVo.isEmpty
      }))
