...
import setPolicyId from './API/setPolicyId';
...
const OpusBPM = ({ children }: any) => {
  ...
  const renderChildren = React.Children.map(children, (child) => {
    if (child?.type?.displayName === 'BPM.HeaderPolicyId') {
      console.log('进入HeaderPolicyId', child);
      OpusBPM.HeaderPolicyId = () => <>{child?.props?.children}</>;
    } else {
      return child;
    }
  });
...
OpusBPM.HeaderPolicyId = (props: any) => <></>;
(OpusBPM.HeaderPolicyId as any).displayName = 'BPM.HeaderPolicyId';
