...
export default async ({
  ...
  if (isAuto && buttonConfig.buttonCode === 'save') {
  ...
  for (let i = blockIndex; i < serviceList.length; i += 1) {
    ...
    if (buttonServiceUrl) {
      ...
      if (
        ...
        isAuto &&
        dataCompare({
          data: { ...(data || {}), platformCode: 'opus' },
          cacheData: autoActionDataCache.get(cacheKey),
        })
      ) {
      ...
      if (!success) {
        finalSuccessResult = false;
        const ignoreErrorMsg = lodash.get(response, 'resultData.ignoreErrorMsg', false);
        if (!ignoreErrorMsg) {
          handleError({
            response,
