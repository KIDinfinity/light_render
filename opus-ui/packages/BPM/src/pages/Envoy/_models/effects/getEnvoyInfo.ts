...
function* getEnvoyInfo({ signal, payload }: any, { select, call, put }: any) {
  const { caseNo, businessNo, activityKey, isCallFindReasonInfo, inquiryBusinessNo } = yield select(
    (state: any) => state.envoyController
  );
  const response = yield call(
    navigatorEnvoyControllerService.findReasonInfoByInquiryBusinessNo,
    {
      inquiryBusinessNo,
      caseNo: caseNo || payload?.caseNo,
      activityKey: !activityKey ? null : activityKey,
    },
    { signal }
  );
