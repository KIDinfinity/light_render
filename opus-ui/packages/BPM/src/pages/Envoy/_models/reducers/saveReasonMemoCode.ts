  ...
  return produce(state, (draftState: any) => {
    ...
    lodash.forEach(draftState.currentReasonGroups, (reasonGroup: any) => {
      if (reasonGroup?.id === groupId) {
        lodash.forEach(reasonGroup.reasonDetails, (reason: any, index: number) => {
          if (reason?.id === dataId) {
            ...
            if (value !== '') {
              ...
              names.map((name) => {
                if (lodash.includes(name, 'memoCode')) {
                  ...
                  lodash.set(
                    obj,
                    `${paths[index]?.split('.')[0]}.pendingMemoSubInfoList[0].subTypeCode`,
                    ''
                  );
                  lodash.set(
                    obj,
                    `${paths[index]?.split('.')[0]}.pendingMemoSubInfoList[0].subRemark`,
                    ''
                  );
                }
