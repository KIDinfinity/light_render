...
interface IProps {
  displayConfig: ReasonDisplayConfig;
}
export default ({ displayConfig }: IProps) => {
  const pendingMemo = displayConfig?.pendingMemo?.children;
  return useMemo(() => {
    ...
    const objects = {
      ...
      memoDesc: {
        visible: true,
        required: lodash.isBoolean(pendingMemo?.memoDesc?.required)
          ? pendingMemo?.memoDesc?.required
          : isRequired('memoDesc'),
        dropDownList: [],
        moduleName: 'memoDesc',
      },
      subTypeCode: {
        visible: pendingMemo?.subTypeCode || false,
        required: pendingMemo?.subTypeCode && isRequired('memoDesc'),
        dropDownList: [],
        moduleName: 'memoDesc',
      },
      requestedClientRole: {
    ...
    return lodash
      ...
      .value();
  }, [displayConfig, pendingMemo?.memoDesc?.required, pendingMemo?.subTypeCode]);
};
