...
export default ({
  ...
  const filterReason = lodash
    ...
    .filter(
      (otherSubTypeCode) =>
        otherSubTypeCode && otherSubTypeCode !== EMemoCode.FREE
    )
    .value();
            ...
            <Form layout="vertical">
                    ...
                    {lodash.map(listMemos, (item) => (
                      ...
                      >
                        {(() => {
                          ...
                          if (memoCodeShowType === 'both')
                            return `${item.memoCode}-${item.memoName}`;
                          if (memoCodeShowType === 'keyAndDesc')
              ...
              <div style={{ paddingLeft: isMultiple? 5 : 0}}>
                {pendingMemoList[`${idx}`].pendingMemoSubInfoList?.map((item, index) => {
                  ...
                  return (
                    <div className={classnames(styles.select, styles.reasonSection)} key={`${item.id}_${index}`}>
                      {
                      ...
                      >
                        <Select
                          ...
                          id="reasonCode"
                        >
                          {lodash.map(DictsReason.filter(item => {
                            const duplicateIndex = pendingMemoList[`${idx}`]?.pendingMemoSubInfoList?.findIndex(subInfo => subInfo.subTypeCode === item.reasonCode);
                      ...
                      {displayConfig.memoRemark && (
                        <Form.Item
                          label={
                            fieldsRequired?.memoRemark? <div className={styles.label}>
