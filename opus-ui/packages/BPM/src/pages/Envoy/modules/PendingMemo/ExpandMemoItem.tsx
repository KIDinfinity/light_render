...
import { formUtils, FormItemInput } from 'basic/components/Form';
  ...
  return useMemo(
    () => (
      <div className={classnames({
        ...
        [styles.withReceive]: showReceive({ status: data?.status, disabled: receivedDisable, memoStatus: memoItem.memoStatus })
      })} key={idx} id={`memo${idx}`}>
        <Read
          type={EType.ITEM}
        ...
        >
          ...
          {displayConfig.memoRemark && (
            <Form.Item
              className={classnames(styles.expandRemark,styles.inside)}
              label={
                <div className={styles.label}>
  ...
  Form.create<any>({
    ...
    mapPropsToFields(props: any) {
      ...
      const obj = formUtils.mapObjectToFields({
        ...
        [`pendingMemoList{${idx}}_pendingMemoSubInfoList{0}_subRemark`]: pendingMemoList[`${idx}`]
          ?.memoRemark,
      });
