...
export default ({ editable, data }: any) => {
  ...
  if (!reasonHaveMemoSubType[data?.reasonCode]) {
    listMemos = listMemos?.filter(
      ({ memoCode }) => !pendingMemoList.some((memoItem) => memoItem.memoCode === memoCode)
    );
  }
          ...
          <>
            ...
            {!disabled && (
              <div className={styles.clientSection}>
                <AddMemoByMemoCode addMemo={addByMemoCode} listMemos={listMemos} isExpand={true} />
              </div>
            )}
          </>
        ) : !isRole ? (
