...
export default function FreeFieldsOfCheckbox({
  required = false,
  editable,
  custom,
  data,
  ...
  const saveData = (value: string) => {
    if (type === EDataType.REASON) {
      dispatch({
        type: 'envoyController/saveFreeFieldsOfCheckboxReasonData',
        payload: {
          groupId,
          dataId: id,
          dataPath,
      ...
      dispatch({
        type: 'envoyController/saveFreeFieldsOfCheckboxReminderData',
        payload: {
          groupId,
          dataId: id,
          dataPath,
  ...
  return (
    ...
    >
      <Checkbox
        ...
        name={name}
        disabled={disabled}
        checked={data[dataPath]}
        onChange={(ev: any) => saveData(ev.target.checked)}
      />
    </Form.Item>
  );
}
