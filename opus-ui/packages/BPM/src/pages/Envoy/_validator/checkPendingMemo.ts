...
const checkPendingMemo = (errObj: any, reason: any, isSendReason: any) => {
  ...
  displayConfig = {
    ...
    ...displayConfig,
    subTypeCode: displayConfig?.subTypeCode,
    subRemark: displayConfig?.subRemark,
    surveyCompany: displayConfig?.surveyCompany,
  ...
  if (isSendReason && hasMemo) {
    ...
    forEach(pendingMemoList, (item: any, idx: number) => {
      rulerList.forEach((ruler) => {
        const isFieldEmpty = ['memoReason', 'memoRemark', 'subTypeCode', 'subRemark'].includes(
          ruler.codeName
        )
          ? lodash.isEmpty(item?.pendingMemoSubInfoList) ||
            lodash.some(item?.pendingMemoSubInfoList, (subItem) =>
              lodash.isEmpty(subItem?.subTypeCode)
            )
          : !item[ruler.codeName];
        ...
        if (['memoReason', 'memoRemark', 'subTypeCode', 'subRemark'].includes(codeName)) {
          if (isError) {
            const index = lodash.findIndex(item?.pendingMemoSubInfoList, (subItem) =>
              ruler.codeName === 'subTypeCode' ? !subItem?.subTypeCode : !subItem?.subRemark
            );
            errObj[
              `pendingMemoList{${idx}}_pendingMemoSubInfoList{${
                index >= 0 ? index : 0
              }}_${codeName}`
            ] = isError ? message : [];
          }
