...
@connect(({ bpOfClaimAssessmentController, claimEditable }: any, { treatmentId }: any) => ({
  curTreatmentPayableList: getTreatmentPayableList(
    treatmentId,
    bpOfClaimAssessmentController.claimEntities.treatmentPayableListMap
  ),
  treatmentPayableAddItem: bpOfClaimAssessmentController.treatmentPayableAddItem,
...
class TreatmentPayableList extends Component {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'bpOfClaimAssessmentController/addTreatmentPayableItem',
      payload: {
        incidentId,
        treatmentId,
        claimNo,
  ...
  render() {
    ...
    return (
      <div>
        {lodash.map(lodash.compact(curTreatmentPayableList), (item) => (
