...
@connect(({ bpOfDataCaptureController, claimEditable }: any, { treatmentId }: anys) => ({
  claimNo: lodash.get(bpOfDataCaptureController, 'claimProcessData.claimNo'),
  procedureList:
    bpOfDataCaptureController.claimEntities.treatmentListMap[treatmentId].procedureList,
...
class ProcedureList extends PureComponent {
  handleAdd = () => {
    ...
    dispatch({
      type: 'bpOfDataCaptureController/addProcedureItem',
      payload: {
        treatmentId,
        addProcedureItem,
      },
