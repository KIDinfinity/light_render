...
jest.mock(
  'opus/NewBusiness/ManualUnderwriting/Pages/Client/_hooks/useGetDefaultJuvenileAgeRange',
  () => {
    return jest.fn(() => {
      return [0, 17];
    });
...
describe('useJudgeCurrentInsuredUnderage', () => {
  test('policyOwner & Insured & match age', () => {
    const renderer = renderHook(() => {
      return useJudgeCurrentInsuredUnderage({
        clientId: '3',
        readOnly: false,
      });
    });
