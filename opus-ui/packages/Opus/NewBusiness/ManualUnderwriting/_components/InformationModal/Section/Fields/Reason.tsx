...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const { typeCode = '', value = [] } = categoryReasons?.length ? categoryReasons[0] : {};

  const miscDicts = getDrowDownList(typeCode);

  // 临时方案，待后端调整getDropdownDictionary接口
  const dicts = lodash.filter(miscDicts, (item: any) =>
    value.some(({ reasonCode }: any) => reasonCode == item.dictCode)
  );

  const initialValue = value.find((item: any) => item.isDefault)?.reasonCode;
