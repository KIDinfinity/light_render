...
const Processing = () => {
  ...
  const isSaveDataComplete = useSelector(
    ({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace?.isSaveDataComplete
  );
  const loadingStatus = useSelector(({ login }: any) => login?.loadingStatus);
  const saveDataFlag = !loadingStatus ? !isSaveDataComplete : false;
  ...
  return (
    <Modal
      ...
      footer={null}
      visible={!lodash.isEmpty(progressData) || saveDataFlag}
      centered
    >
