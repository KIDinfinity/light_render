...
import useGetPrimaryAgentChannel from 'opus/NewBusiness/ManualUnderwriting/Pages/Client/_hooks/useGetPrimaryAgentChannel';
import useGetSomeBeneficiaryUnderage from 'opus/NewBusiness/ManualUnderwriting/_hooks/useGetSomeBeneficiaryUnderage';
import useGetSomeInsuredUnderage from 'opus/NewBusiness/ManualUnderwriting/_hooks/useGetSomeInsuredUnderage';
import useJudgeMainCoverageIsTL from 'opus/NewBusiness/ManualUnderwriting/_hooks/useJudgeMainCoverageIsTL';
...
export default ({ id, readOnly }: any) => {
  ...
  return useMemo(() => {
    ...
    // 当前客户的角色
    const customerType = lodash
      .chain(clientList)
      .filter((clientId: string) => id === clientId)
      .map((clientId: string) =>
        formUtils.queryValue(clientMap[clientId]?.personalInfo?.customerType)
      )
      .flatten()
      .value();
