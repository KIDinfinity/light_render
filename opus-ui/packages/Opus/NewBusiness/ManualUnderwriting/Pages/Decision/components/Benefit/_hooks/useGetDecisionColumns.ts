...
import useGetFacultativeOptionVisible from 'decision/_hooks/useGetFacultativeOptionVisible';
import useGetFacultativeReasonVisible from 'decision/_hooks/useGetFacultativeReasonVisible';
...
export default () => {
  ...
  const isDisplayFacultativeOption = useGetFacultativeOptionVisible();
  const isDisplayFacultativeReason = useGetFacultativeReasonVisible();
  ...
  const result = useMemo(() => {
    ...
    return lodash
      ...
      .filter((fieldConfig: any) => {
        const field = lodash.get(fieldConfig, 'field');
        return !(field === 'facultativePackageCode' && !isDisplayFacultativeOption);
      })
      .filter((fieldConfig: any) => {
        const field = lodash.get(fieldConfig, 'field');
        return !(field === 'facultativeReason' && !isDisplayFacultativeReason);
      })
      .map((fieldConfig: any) => {
    ...
    isDisplayFacultativeOption,
    isDisplayFacultativeReason,
  ]);
