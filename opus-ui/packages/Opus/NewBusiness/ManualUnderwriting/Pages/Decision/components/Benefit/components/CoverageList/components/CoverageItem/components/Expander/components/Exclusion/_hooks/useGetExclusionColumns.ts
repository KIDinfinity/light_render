...
import getExclusionHiddenColumn from 'packages/Opus/NewBusiness/ManualUnderwriting/_utils/getExclusionHiddenColumn';
...
export default () => {
  ...
  const hiddenConditionColumnList = getExclusionHiddenColumn(config);
  return useMemo(() => {
    return lodash
      .chain(config)
      .filter((item) => item?.['field-props']?.visible !== 'N')
      .filter((item) => !hiddenConditionColumnList.find((ele) => item?.field === ele))
      .map((item) => {
