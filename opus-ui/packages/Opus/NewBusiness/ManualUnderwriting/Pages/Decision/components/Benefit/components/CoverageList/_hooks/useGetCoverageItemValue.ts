...
// TODO:
import useGetRopListByCoreCode from 'opus/NewBusiness/ManualUnderwriting/Pages/Decision/components/Benefit/_hooks/useGetRopListByCoreCode.ts';
...
export default ({ item, col }: any) => {
  ...
  const { dictCode } = item.id
    ? useGetNumberofunitsDictsByProductCode({ id: item?.id })
    : { dictCode: 'numberOfUnits' };
  const dicts = useGetNumberofunitsDictsByProductCode({ id: item?.id });
