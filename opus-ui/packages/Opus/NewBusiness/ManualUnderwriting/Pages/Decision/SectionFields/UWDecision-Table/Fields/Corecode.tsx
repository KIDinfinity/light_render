...
import useGetProductDicts from 'opus/NewBusiness/ManualUnderwriting/_hooks/useGetProductDicts';
import usehandleProductNameChangeCallback from 'decision/Edit/_hooks/CoreCode/usehandleProductNameChangeCallback';
import useGetClientDetailList from 'opus/NewBusiness/ManualUnderwriting/_hooks/useGetClientDetailList';
...
import { MessageType } from 'opus/NewBusiness/ManualUnderwriting/_enum/medicalSearchMessageType';
import useGetProductMaxNoLimit from 'decision/Edit/_hooks/CoreCode/useGetProductMaxNoLimit';
...
const FormItem = ({
  ...
  const currentCoreCode = form.getFieldValue(config?.['field-props'].name || field || 'coreCode');
  ...
  const propsConfig = {
    field,
    editable: !disabled && editable && !riderRequiredInd && !disabledForMain,
    isShow,
    form,
    Rules,
  ...
  const { calculatedEditable, calculatedVisible, calculatedRequired, label, name, calculatedRules } =
