...
import { fieldConfig } from './EpfWithdrawalRefId.config';

export { fieldConfig } from './EpfWithdrawalRefId.config';
...
const FormItem = ({ isShow, layout, form, editable, field, config, showOnly }: any) => {
  ...
  const visibleConditions = useFamilyGroupIndInclude({groupInd: ['P','E']});
...
const EpfWithdrawalRefId = ({ field, form, editable, layout, isShow, config,showOnly }: any) => (
  <Authority>
      <FormItem field={field} isShow={isShow} layout={layout} form={form} editable={editable} config={config} showOnly={showOnly}/>
  </Authority>
);

EpfWithdrawalRefId.displayName = 'epfWithdrawalRefId';

export default EpfWithdrawalRefId;
