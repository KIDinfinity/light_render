...
import useJudgeIsCoInsured from './useJudgeIsCoInsured';
...
export default ({ config, fieldConfig, contactSeqNum, id, readOnly }: any) => {
  ...
  let resultDicts = defaultDicts;

  const isCoInsured = useJudgeIsCoInsured({ clientId: id });

  if (isCoInsured) {
    resultDicts = lodash.filter(defaultDicts, (item) => ['MB'].includes(item.dictCode));
  }
