...
const FormItem = ({ isShow, layout, form, editable, field, config, id }: any) => {
  ...
  const shouldExcludeEntity =
    !customerRole?.includes(CustomerRole.Payor) && customerRole?.length > 0;

  if (
    shouldExcludeEntity ||
    (customerRole && customerRole.some((role: string) => role !== CustomerRole.Payor))
  ) {
    dicts = dicts.filter((item: any) => item.dictCode !== CustomerType.Entity);
  }
