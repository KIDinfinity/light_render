  ...
  return lodash.reduce(
    ...
    (result: any, fragment: any) => {
      ...
      if (rtCoverage) {
        ...
        result.rtCoverage.push(rtCoverage);
      }
      return result;
    },
    {
...
const handleUpdateCoverageList = ({ draftState, optionItem }: any) => {
  ...
  if (draftState?.processData?.policyId === optionItem?.policyId) {
    ...
    const concatCoverageList = lodash.filter(coverageFragment?.rtCoverage, (rtCoverage: any) =>
      lodash.every(coverageList, (coverage: any) => coverage?.id !== rtCoverage?.id)
    );
    return lodash
...
export default (state: any, { payload }: any) => {
  const optionItem = lodash.get(payload, 'optionItem', {});
