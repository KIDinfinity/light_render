...
import { Modal, Icon, Button, Alert } from 'antd';
...
class ModalWarnMessage extends PureComponent<IProps, IState> {
  ...
  title = () => {
    ...
    let massage = { Label_COM_Opus: 'confirmation' };

    switch (type as any) {
      case 'error':
        massage = { Label_COM_General: 'Error' };
        break;
      case 'success':
        massage = { Label_COM_Opus: 'Success' };
        break;
    }

    return (
      <>
        <Icon type={type === 'success' ? 'check-circle' : 'exclamation-circle'} />
        <span>{formatMessageApi(massage)}</span>
      </>
    );
  };
      ...
      <>
        ...
        {!hideCancelButton && (
          <Button
            ...
            className={cancelClassName || tabIndex === ModalFootButton.cancel ? 'active' : ''}
            {...cancelButtonProps}
          >
            {cancelText}
          </Button>
