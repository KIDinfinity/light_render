...
const EnvoyRow = ({ children, showMedicalProvider }: any) => {
  const spans = showMedicalProvider ? [5, 5, 5, 4, 5] : [5, 7, 0, 7, 5];

  return (
    <Row style={{ marginTop: '10px' }}>
      {Children.map(children, (child, index) => {
        if (index === 2 && !showMedicalProvider) {
          return null;
        }
        return (
            ...
            (!isNotHavePendingMemo ? (
              <div className={styles.memoWrap}>
                <div style={{ marginTop: '10px' }}>
                  <EnvoyRow showMedicalProvider={showMedicalProvider}>
                  ...
                  <div>
                    {item?.reasonDetails?.flatMap(({ pendingMemoList, reasonCode, groupId }) => {
                      return pendingMemoList?.map((memo) => (
                        <>
                          <EnvoyRow key={memo.id} showMedicalProvider={showMedicalProvider}>
                            {getPaddingCode(memo)}
