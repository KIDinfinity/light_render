...
const DocumentViewer: FunctionComponent<IDocumentViewer> = () => {
  const dispatch = useDispatch();
  const [loading, setLoading] = useState(true);
  const { documentList, businessNoDocumentList, showType, selectedId, fileObject } = useSelector(
    ({ documentManagement }: any) => ({
      ...
      selectedId: documentManagement.selectedId,
      fileObject: documentManagement.fileObject,
    })
  );
  const currentDocumentItem = businessNoDocumentList.find((item) => item?.id === selectedId);
  ...
  // item non-visible when voided
  if (currentDocumentItem?.voidFlag === 1 && tenant.isJP()) {
    return <></>;
  }
