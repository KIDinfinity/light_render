...
const Item = ({ otherProcedureId, antiCancerProcedureItem, idx }: any) => {
  ...
  const deleteDates = (therapeuticDate: Date) => () => {
    dispatch({
      type: 'opusClaimDataCapture/therapeuticDateListDelete',
  ...
  const deleteMonth = () => () => {
    dispatch({
      type: 'opusClaimDataCapture/therapeuticMonthListDelete',
            ...
            {lodash.size(therapeuticDateList || []) > 1 && !!editable && (
              <div className={styles.close}>
                <Icon
                  type="close"
                  onClick={() =>
                    showDeleteConfirmModal({ onConfirm: deleteDates(therapeuticDate) })
                  }
                />
              </div>
            )}
