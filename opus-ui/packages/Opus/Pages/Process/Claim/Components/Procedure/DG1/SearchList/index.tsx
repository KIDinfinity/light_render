...
export default (props: any) => {
  ...
  const DrugsDetail =
    useSelector(({ opusClaimDataCapture }: any) => opusClaimDataCapture.DrugsDetail) || {};
  const antiCancerProcedureItem = useSelector(
    ({ opusClaimDataCapture }: any) =>
      opusClaimDataCapture.claimEntities.otherProcedureListMap[otherProcedureId]
  ...
  const handleCancel = () => {
    dispatch({
      type: 'opusClaimDataCapture/updateDrugsDetailList', //关闭并清空
      payload: {
  ...
  const handleConfirm = async () => {
    ...
    await dispatch({
      type: 'opusClaimDataCapture/saveDrugsDetailList', //保存已选中药品
      payload: {
        therapeuticMonth,
        otherProcedureId,
        changedFields: {
