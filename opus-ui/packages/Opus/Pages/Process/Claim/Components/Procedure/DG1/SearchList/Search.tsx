...
const Search = ({ dispatch, payableData, form, showSearch }: IProps) => {
  const handleReset = async () => {
    form.resetFields();
    await dispatch({
      type: 'opusClaimDataCapture/saveSearchListParams', //清空已保存的搜索字段
      payload: {
        changedFields: [],
      },
    });
    await dispatch({
      type: 'opusClaimDataCapture/getDrugsDetailList', //请求数据
      payload: {
        currentPage: 1,
      },
    });
        ...
        <div className={styles.Search}>
          ...
          <div className={styles.buttonGroup}>
            <Button
              ...
              onClick={() => {
                dispatch({
                  type: 'opusClaimDataCapture/getDrugsDetailList',
                  payload: {
                    searchState: true,
                  },
                });
...
export default connect((state: any) => ({
  ...
  filterParams: state?.opusClaimAssessment?.serialClaim?.filterParams,
  payableData: state?.opusClaimAssessment?.serialClaim?.payableData,
}))(
  Form.create<any>({
    onFieldsChange(props: any, changedFields: any) {
      const { dispatch } = props;
      dispatch({
        type: 'opusClaimDataCapture/saveSearchListParams', //onChange时保存搜索字段
        payload: {
