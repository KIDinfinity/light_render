...
export default function* getDrugsDetailList({ payload }: any, { call, select, put }: any) {
  const filterParams = yield select(
    (state: any) => state.opusClaimDataCapture.DrugsDetail.filterParams
  );
  const searchState: any = yield select(
    (state: any) => state.opusClaimDataCapture.DrugsDetail.searchState
  );
  let responseInfo = [];
  const currentPage = payload?.currentPage;
  if (!lodash.isEmpty(filterParams) && payload?.searchState) {
    //点击搜索
    responseInfo = yield call(getStandardDrugListForPage, {
      params: filterParams,
      currentPage: 1,
      pageSize: 5,
    });
  ...
  } else if (!lodash.isEmpty(filterParams) && searchState) {
    //搜索时分页
    responseInfo = yield call(getStandardDrugListForPage, {
      params: filterParams,
      currentPage,
