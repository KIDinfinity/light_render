...
export default function* ({ payload }: any, { call, put, select }: any) {
  ...
  const { searchInsuredObj, caseCategory, businessNo, taskDetail } = yield select((state: any) => ({
    searchInsuredObj: state.opusNonOpusClaimManagement.searchInsuredObj,
    caseCategory: state?.opusNonOpusClaimManagement?.businessData?.caseCategory,
    businessNo: state?.opusNonOpusClaimManagement?.businessData?.businessNo,
