...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const policyOwnerList = useSelector(
    ({ opusNonOpusClaimManagement }: any) => opusNonOpusClaimManagement.policyOwnerList
  );

  const changeRelationship = (value: any) => {
    if (lodash.isEmpty(policyOwnerList)) {
      dispatch({
        type: `opusNonOpusClaimManagement/getPolicyOwnerList`,
        payload: {},
      });
    }
  };
