...
const FormItem = ({
  ...
  const setDateListOrdered = (nextDateList) => {
    setDateList(lodash.orderBy(nextDateList, (item) => moment(item).valueOf(), ['asc']));
  };
  ...
  const incidentId = useSelector(
    ({ opusClaimAssessment }: any) =>
      opusClaimAssessment.claimEntities?.treatmentListMap?.[treatmentId]?.incidentId
  );
  ...
  const onConfirm = () => {
    dispatch({
      type: 'opusClaimAssessment/opTreatmentListAdd',
      payload: {
        treatmentId,
        groupId,
        incidentId,
  ...
  const onChange = (date: string) => {
    ...
    setDateListOrdered([...dateList, date]);
  };
  ...
  const deleteDates = (deleteDate?: Date) => () => {
    dispatch({
      type: 'opusClaimAssessment/opTreatmentListDelete',
      payload: {
        treatmentId,
        deleteDate,
      },
