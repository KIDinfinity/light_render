...
const localFieldConfig = {
  ...
  field: 'procedureDateOCR',
  'field-props': {
    editable: 'N',
    label: {
      dictTypeCode: 'Label_BIZ_Claim',
      dictCode: 'OCRProcedureDate',
    },
    maxLength: 240,
    required: 'N',
    visible: 'Y',
...
const FormItem = ({ isShow, layout, form, editable, config, field, incidentId }: any) => {
  ...
  const getIncidentDate = ({ [NAMESPACE]: modelnamepsace }: any) =>
    modelnamepsace.claimEntities.incidentListMap[incidentId]?.incidentDate;
  const getDateTimeOfDeath = ({ [NAMESPACE]: modelnamepsace }: any) =>
...
const ProcedureDateOCR = ({ field, config, form, editable, incidentId, layout, isShow }: any) => (
  <Authority>
    <FormItem
      field={field}
      config={config}
...
ProcedureDateOCR.displayName = 'ProcedureDateOCR';

export default ProcedureDateOCR;
