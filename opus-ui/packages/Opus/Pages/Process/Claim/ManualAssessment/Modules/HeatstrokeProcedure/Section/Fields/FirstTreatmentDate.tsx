...
const FormItem = ({ isShow, layout, form, editable, field, config, otherProcedureItem }: any) => {
  ...
  const onConfirm = (_: any, list?: any[]) => {
    ...
    dispatch({
      type: 'opusClaimAssessment/firstTreatmentDateAdd',
      payload: {
        otherProcedureId: otherProcedureItem?.id,
        treatmentDates: lodash.map(dates, (item) => moment(item).valueOf()),
      },
    });
    setOpen(false);
  };
  ...
  const parsedFooterDateList = lodash
    .map(otherProcedureItem?.therapeuticMonthList, (item) => item.firstTreatmentDate)
    .filter((date) => !!date);
  const deleteDates = (treatmentDate: Date) => () => {
    dispatch({
      type: 'opusClaimAssessment/firstTreatmentDateDelete',
      payload: { otherProcedureId: otherProcedureItem?.id, treatmentDate },
    });
  };
...
const OutpatientDate = ({ form, editable, section, layout, isShow, otherProcedureItem }: any) => (
  <Authority>
    <ElementConfig.Field
