...
import { clearClaimPayableItem, updateDuplicatePayableError } from '../functions';
import { getMainProductCodeByPolicyNo } from 'claim/pages/utils/claimPayableFunc';

const saveClaimPayableItem = (state: any, action: any) => {
  const { listPolicy } = state;
  const { changedFields, incidentPayableId, policyNo } = action.payload;
  ...
  const nextState = produce(state, (draftState) => {
    ...
    const claimPayableListMap = newClaimEntities?.claimPayableListMap;
    ...
    const { treatmentPayableList, incidentId } =
      newClaimEntities.claimPayableListMap[incidentPayableId];
    ...
    if (fieldsArray.length === 1) {
      ...
      if (name === 'policyNo' && !dirty) {
        ...
        if (!isEmpty(value)) {
          editPayableItem.mainProductCode = getMainProductCodeByPolicyNo(
            listPolicy,
            editPayableItem
          );
        }
      ...
      if (name === 'benefitTypeCode' && !dirty) {
        ...
        const calculateByPolicyYearlist = ['F', 'Y'];
        ...
        if (
          payable.length === 0 ||
          lodash.includes(calculateByPolicyYearlist, mappingPolicy?.calculateByPolicyYear)
        ) {
          ...
          editPayableItem.calculateByPolicyYear = get(mappingPolicy, 'calculateByPolicyYear');
          editPayableItem.benefitSubCategory = get(mappingPolicy, 'benefitSubCategory');
          ...
          payable.length > 0 &&
          get(mappingPolicy, 'benefitCategory') === BenefitCategory.MajorIllnessCashBenefit
        ) {
          const result = complementClaimPayableItem(
            payable[0][1],
            newClaimEntities,
          ...
          newClaimEntities = result.editClaimEntities;
          newClaimEntities.claimPayableListMap[payable[0][0]] = result.editClaimPayableListItem;
        }
        // submit的时候 duplicate的payable会出现error 当change其中一个payable的时候 需要清空另一个payable的error
        updateDuplicatePayableError(
          draftState,
          {
            editPayable: { ...editPayable, benefitTypeCode: preBenefitTypeCode },
          },
          'claimPayable'
        );
      }
      ...
      if (name === 'claimDecision') {
        if (value === ClaimDecision.deny && incidentPayableItem?.isAdjustment !== 'Y') {
          ...
          forEach(treatmentPayableList, (treatmentPayableId) => {
            ...
            const { procedurePayableList } = treatmentPayable;
            forEach(procedurePayableList, (procedurePayableId) => {
              const procedurePayableItem =
                newClaimEntities.procedurePayableListMap[procedurePayableId];
              procedurePayableItem.systemCalculationAmount = 0;
              procedurePayableItem.assessorOverrideAmount = null;
              procedurePayableItem.payableAmount = 0;
              procedurePayableItem.reimbursementMultiple = null;
              procedurePayableItem.assessorOverrideMultiple = null;
              newClaimEntities.procedurePayableListMap[procedurePayableId] = procedurePayableItem;
            });
            const { opTreatmentPayableList } = treatmentPayable;
            forEach(opTreatmentPayableList, (opTreatmentPayableId) => {
              const opTreatmentPayableItem =
                newClaimEntities.opTreatmentPayableListMap[opTreatmentPayableId];
              opTreatmentPayableItem.payableAmount = 0;
              opTreatmentPayableItem.payableDays = null;
              opTreatmentPayableItem.assessorOverrideDays = null;
              opTreatmentPayableItem.assessorOverrideAmount = 0;
              opTreatmentPayableItem.hospitalizationSequentialNo = null;
              newClaimEntities.opTreatmentPayableListMap[opTreatmentPayableId] =
                opTreatmentPayableItem;
            });

            const { otherProcedurePayableList } = treatmentPayable;
            forEach(otherProcedurePayableList, (procedurePayableId) => {
              const otherProcedurePayableItem =
                newClaimEntities.otherProcedurePayableListMap[procedurePayableId];
              otherProcedurePayableItem.systemCalculationAmount = 0;
              otherProcedurePayableItem.assessorOverrideAmount = null;
              otherProcedurePayableItem.payableAmount = 0;
              otherProcedurePayableItem.reimbursementMultiple = null;
              otherProcedurePayableItem.assessorOverrideMultiple = null;
              newClaimEntities.procedurePayableListMap[procedurePayableId] =
      ...
      if ((name === 'settlementDecision' || name === 'detailedAssessmentDecision') && !dirty) {
        const key = Object.keys(changedFields)[0];
        lodash.map(claimPayableListMap, (claimPayable, claimPayableId) => {
          if (
            incidentId &&
            lodash.isEqual(incidentId, claimPayable?.incidentId) &&
            lodash.isEqual(policyNo, claimPayable?.policyNo)
          ) {
