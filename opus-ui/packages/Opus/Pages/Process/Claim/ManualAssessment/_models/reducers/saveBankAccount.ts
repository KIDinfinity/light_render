          ...
          const bankAccounts = lodash
            ...
            .map((bankAccount: BankAccountModal) => {
              if (bankAccount.id === bankAccountId) {
                if(isEditStatus && lodash.has(changedFields, 'bankCode')) {
                  if(formUtils.queryValue(payee.paymentMethod) === PaymentMethod.PremiumAccount && formUtils.queryValue(changedFields.bankCode)) {
                    changedFieldsTemp.bankType = formUtils.queryValue(changedFields.bankCode) === '9900'? 'POST': 'BANK';
                  }
                }
