...
export default function* validateFields(_, { select, put }: any) {
  ...
  const claimProcessData = yield select((state: any) => state.opusClaimAssessment.claimProcessData);
  const draftState = yield select((state: any) => state.opusClaimAssessment);
  const claimEntities = yield select((state: any) => state.opusClaimAssessment.claimEntities);
