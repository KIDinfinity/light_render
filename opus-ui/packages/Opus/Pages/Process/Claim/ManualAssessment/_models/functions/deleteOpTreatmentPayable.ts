...
export default ({ draftState, treatmentId, outpatientTreatmentDate, diagnosisCodes }: any) => {
  return lodash
    .filter(draftState.claimEntities.opTreatmentPayableListMap, (payable) => {
      ...
      if (diagnosisCodes) {
        return (
          payable.treatmentId === treatmentId &&
          !lodash.includes(
            formUtils.cleanValidateData(diagnosisCodes),
            formUtils.queryValue(payable?.diagnosisCode)
          )
        );
      }
      return false;
    ...
    .forEach((payable) => {
      draftState.claimEntities.treatmentPayableListMap[payable.parentId].opTreatmentPayableList =
        lodash.filter(
          draftState?.claimEntities?.treatmentPayableListMap?.[payable?.parentId]
