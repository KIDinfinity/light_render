...
import { NAMESPACE } from 'packages/Opus/Pages/Process/Claim/DataCapture/activity.config.ts';
...
export default connect(({}: any) => ({}))(
  Form.create({})(({ form }: any) => {
    ...
    const { show, searchList, searchContent, treatmentId, procedureId, current, total } =
      useSelector(({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.procedureModal) || {};
    ...
    const onClose = async () => {
      await dispatch({
        type: `${NAMESPACE}/procedureModalHidden`,
      });
    };
    ...
    const selectData = (item: any) => {
      ...
      dispatch({
        type: `${NAMESPACE}/procedureUpdate`,
        payload: {
          changedFields: {
            ...changedFields,
            procedureName: item.approvalProcedureName,
          },
          treatmentId,
          procedureId,
        },
