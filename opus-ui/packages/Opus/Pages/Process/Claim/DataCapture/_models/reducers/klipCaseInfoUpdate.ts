...
const getpolicyListValue = ({ draftState, changedFields }: any, fieldName: string) => {
  return lodash
    .chain(draftState.policyList || [])
    .find((el: any) => el.policyId === formUtils.queryValue(changedFields.policyId))
    .get(fieldName)
    .value();
};
...
const klipCaseInfoUpdate = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (lodash.size(changedFields) === 1) {
      ...
      const klipCaseInfoList = draftState.integration[incidentId].klipCaseInfoList;
      const claimTypeArray = formUtils.queryValue(
        draftState.claimEntities?.incidentListMap?.[incidentId]?.claimTypeArray
      );
      draftState.integration[incidentId].klipCaseInfoList = lodash.map(
        klipCaseInfoList,
        (item: any) => {
          let extra = {};
