...
const saveSelectInsuredInfo = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    const policyOwnerList = lodash.get(draftState, 'policyOwnerList', []);
    const policyId = formUtils.queryValue(draft.claimProcessData.insured.policyId);
    const policyOwner = policyOwnerList.find((item) => item?.policyId === policyId);
    if (policyOwner) {
      draftState.claimProcessData.owner = {
        ...policyOwner?.ownerClientInfo,
      };
    }
    ...
    if (!lodash.isEqual(oldInsuredId, newInsuredId)) {
      ...
      if (payeeItem.payeeBankAccountList?.[0] && payeeItemData.surname)
        payeeItem.payeeBankAccountList[0].accountHolder = payeeItemData.surname;
      ...
      lodash.set(draft, `claimEntities.payeeListMap.${payeeId || id}`, {
        ...
        ...payeeItemData,
        paymentMethod: '',
        accountHolder: payeeItemData.surname || '',
        paymentType: payeeItem?.paymentType || PaymentType.URGE,
        bizClientId:
          selectColumns?.policySource === SourceSystem.Lifej ? selectColumns.systemClientId : null,
      });
    }
  });
