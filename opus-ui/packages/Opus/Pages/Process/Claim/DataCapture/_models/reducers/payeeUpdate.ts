...
const payeeUpdate = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (lodash.size(changedFields) > 1 && !auto) {
      draftState.claimEntities.payeeListMap[payeeId] = saveDefaultPayeeV2(payeeTemp);
      return;
    }
    ...
    if (!lodash.has(changedFields, 'payeeType')) {
      ...
      const newPayeeTemp = saveDefaultPayeeV2(payeeTemp, changedFields);
    ...
    if (
      ...
      lodash.get(payeeListMap, [payeeId, 'payeeBankAccountList', 0, 'accountType']) === '03'
    ) {
      lodash.set(
        draftState,
        ['claimEntities', 'payeeListMap', payeeId, 'payeeBankAccountList', 0, 'accountType'],
        null
      );
    }
  });
