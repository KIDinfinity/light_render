...
export default function* getC360Data({ payload }: any, { call, put, select }: any) {
  ...
  if (success && resultData) {
    const { policyAgentList = [], clientInfoList = [], policyContractList = [] } = resultData || {};
    const newAgentNoList = getAgentNoList(policyAgentList);
    const newPolicyAgent = getAgentInfo(policyId, policyAgentList);
    ...
    const {
      ...
      sourceSystem = '',
      policySource = '',
    } = lodash.find(policyContractList, { policyId }) || {};
    ...
    yield put({
      ...
      payload: {
        changedFields: {
          policySource: !!policySource ? policySource : sourceSystem,
        },
      },
    });
