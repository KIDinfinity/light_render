...
@Form.create({
  async onFieldsChange(props, changedFields) {
    ...
    if (!validating || lodash.size(changedFields) === 1) {
      dispatch({
        type: `opusClaimDataCapture/saveSearchInsuredInfo`,
        payload: {
          changedFields,
        },
      });
...
class SearchInsuredForm extends Component<IProps> {
  ...
  searchInsured = () => {
    ...
    dispatch({
      type: `opusClaimDataCapture/getInsuredInfo`,
...
export default connect(({ opusClaimDataCapture, dictionaryController, claimEditable }: any) => ({
  searchInsuredObj: opusClaimDataCapture.searchInsuredObj,
