...
class SelectInsuredForm extends Component<IProps> {
  ...
  handleSelectInsured = async () => {
    ...
    await dispatch({
      type: 'opusClaimDataCapture/policyListUpdate',
      payload: { policyList },
    });

    await dispatch({
      type: `opusClaimDataCapture/savePartyListInfo`,
      payload: {
        policyOwnerList: policyResultList,
      },
    });
    await dispatch({
      type: `opusClaimDataCapture/saveSelectInsuredInfo`,
      payload: {
        selectColumns,
        taskDetail,
      },
    ...
    dispatch({
      type: 'opusClaimDataCapture/saveBusinessProcess',
      payload: {
        selectColumns,
      },
    });
    await dispatch({
      type: `opusClaimDataCapture/getC360Data`,
    });
    this.hideShowModal();
  };
...
export default connect(({ opusClaimDataCapture, claimEditable, processTask }: any) => ({
  insuredList: opusClaimDataCapture.insuredList,
