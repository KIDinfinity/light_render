...
const OpusHeaderComponent = () => {
  const policyId = useSelector(
    (state: any) => state[NAMESPACE]?.processData?.policyId
  );
...
const Entry = ({ taskDetail, businessData }: any) => {
  ...
  const policyId =
    useSelector((state: any) => state[NAMESPACE]?.processData?.policyId) || "";
  ...
  return (
    <NamespaceProvider namespace={NAMESPACE}>
      <OpusBPM>
        <OpusBPM.HeaderPolicyId>{policyId}</OpusBPM.HeaderPolicyId>
        {businessData && (
          <ProcessIndex taskDetail={taskDetail} businessData={businessData} />
        )}
        <EntryErrorsUpdate />
      </OpusBPM>
    </NamespaceProvider>
