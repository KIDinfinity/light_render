...
import useUpdateOccupationGroup from '../../../../_hooks/useUpdateOccupationGroup';

const fieldConfig = {
  section: 'payorOccupation',
...
const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  const isHistory = window.location.pathname.toLowerCase().includes('/nb/history');
  const fieldProps: any = fieldConfig?.['field-props'];
  const fullDicts = getDrowDownList({ config, fieldProps });
  const allDicts = getDrowDownList('Dropdown_IND_Occupation');
  const dicts = isHistory ? allDicts : fullDicts;
  useAutoLoadOccupationSubDict(form.getFieldValue('otherOccupation'), 'Dropdown_IND_Occupation');
  useUpdateOccupationGroup({ form, isInsured: false, isSecondary: true })
