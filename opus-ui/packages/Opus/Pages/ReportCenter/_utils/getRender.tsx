    ...
    text: {
      render: (text: any) => {
        return getGroupRender({
          ...
          callback: (el: string) => {
            if (typeof dictionary[fieldName] === 'string') {
              return formatMessageApi({ [dictionary[fieldName]]: el });
            }

            let result = '';
            lodash.forEach(dictionary[fieldName], (item) => {
              const formatted = formatMessageApi({ [item]: el });
              if (formatted && formatted !== el) {
                result = formatted;
              }
            });
      ...
      render: (text: any) =>
        getGroupRender({
          ...
          callback: (date: string) =>
            moment(date).isValid()
              ? moment(date).format(DateFormat[format] || !!format ? format : 'L')
              : date,
        }),
    },
    date_time: {
      render: (text: any) => {
        return getGroupRender({
          ...
          callback: (date: any) =>
            moment(date).isValid()
              ? moment(date).format(DateFormat[format] || !!format ? format : 'L LTS')
              : date,
        });
      },
    },
