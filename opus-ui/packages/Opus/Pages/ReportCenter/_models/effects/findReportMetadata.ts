...
export default function* ({ payload }: any, { call, put, select }: any) {
  const { reportCode, params } = payload;
  ...
  if (!lodash.get(reportMetadata, `${reportCode}`)) {
    const response = yield call(findReportMetadata, { reportCode });
    if (response?.success) {
      ...
      newReportMetadata = {
        ...
        [reportCode]: {
          ...
          dictionary: lodash.reduce(
            ...
            (result, item) => {
              const temp = result;
              // 解决同一fieldName可能对应多个miscType的问题
              if (!temp[item?.fieldName]) {
                temp[item?.fieldName] = item?.miscType;
              } else if (lodash.isArray(temp[item?.fieldName])) {
                temp[item?.fieldName] = [...temp[item?.fieldName], item?.miscType];
              } else {
                temp[item?.fieldName] = [temp[item?.fieldName], item?.miscType];
              }
              return temp;
