...
function ImportData(props: ComponentProps) {
  ...
  const importLoading: boolean = useSelector((state: any) => state.configurationExcel.importLoading);
  const {
    functionData: { operationList,dataFieldList = [] },
  } = props;
  const handlerImport = async ({ file }: any) => {
    await dispatch({
      type: 'configurationExcel/changeImportLoading',
      payload: {
        importLoading: true,
      },
    });
    ...
    await dispatch({
      type: 'configurationExcel/showExcelModal',
      payload: {
        excelData,
      },
    });
  ...
  return (
    <>
      {lodash.includes(operationList, 'importdata') ? (
        <Upload
