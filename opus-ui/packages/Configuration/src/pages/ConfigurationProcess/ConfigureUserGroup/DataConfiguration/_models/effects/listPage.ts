...
export default [
  function* ({ payload = {} }: any, { put, call, select }: any) {
    ...
    const {
      ...
      functionData: state.configureUserGroupController?.functionData,
      caseNo: state.processTask.getTask?.processInstanceId,
      isUpdate: state.configureUserGroupController?.isUpdate,
          ...
          (newList, item): any => {
            ...
            lodash.forEach(subSectionList, (Fitem: any) => {
              const exist = lodash.some(
                newList,
                (nItem) => nItem?.data?.role_code === Fitem?.data?.role_code
              );
              if (!exist) {
