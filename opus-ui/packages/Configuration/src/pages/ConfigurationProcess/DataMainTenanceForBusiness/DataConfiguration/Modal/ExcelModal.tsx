...
import type { ExcelDataProps } from 'configuration/pages/ConfigurationCenter/Utils/Typings';
...
function ExcelModal(props: ComponetProps) {
  ...
  const showExcelModal: boolean = useSelector(
    (state: any) => state.dataConfigurationController.showExcelModal
  );
  const excelData: ExcelDataProps = useSelector(
    (state: any) => state.dataConfigurationController.excelData
  );
