...
function WarnModal(props: any) {
  ...
  const showWarnModal: any = useSelector(
    (state: any) => state.configureRoleController?.showWarnModal
  );
  const isAdd: any = useSelector((state: any) => state.configureRoleController?.isAdd);
  const { form } = props;
  ...
  const onCancel = () => {
    dispatch({
      type: 'configureRoleController/hideWarnModal',
    });
    dispatch({
...
export default connect(({ configureRoleController, claimEditable }: any) => ({
  headerData: configureRoleController?.headerData,
}))(
  Form.create({
    onFieldsChange(props: any, changedFields: any) {
      const { dispatch } = props;
      dispatch({
        type: 'configureRoleController/saveHeaderData',
        payload: {
          changedFields,
        },
      });
