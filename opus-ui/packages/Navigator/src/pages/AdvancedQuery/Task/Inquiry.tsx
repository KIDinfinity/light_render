...
class InquireForm extends Component<any> {
  ...
  componentDidMount() {
    const { stateOfSearch } = this.props;
  ...
  render() {
    ...
    const {
      Dropdown_COM_SubmissionChannel: submissionChannelList,
    ...
    const finalSubmissionChannelList = tenant.region({
      [Region.HK]: hkSubmissionChannelList || [],
      [Region.VN]: submissionChannelList?.filter((item) => item.dictName === 'Smart') || [],
