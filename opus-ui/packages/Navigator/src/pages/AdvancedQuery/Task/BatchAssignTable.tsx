...
const TaskTable = (props) => {
  const { currentActivityList, assigneeList, businessTypeList } = props;
  ...
  const caseCategoryList = getDrowDownList('Label_BPM_CaseCategory');
  ...
  const handleFilterDropdownList = (batchAssignDropDownList = [], oldDropDownList: any) => {
    return lodash.flatten(
      lodash.map(batchAssignDropDownList, (filterItem) =>
        lodash.filter(
          oldDropDownList,
          (item) => (item?.dictCode || item?.userName) === filterItem?.[Object.keys(filterItem)[0]]
        )
      )
    );
  };
  const filterObj = {
    ...
    submissionChannel: handleFilterDropdownList(
      batchAssignDropDownAllList?.submissionChannelList,
      submissionChannel
    ),
    taskOwner: [
      { userId: 'unassigned', userName: 'unassigned' },
      ...(handleFilterDropdownList(batchAssignDropDownAllList?.assigneeList, assigneeList) || []),
...
export default connect(({ advancedQueryBatchAssign }: any) => ({
