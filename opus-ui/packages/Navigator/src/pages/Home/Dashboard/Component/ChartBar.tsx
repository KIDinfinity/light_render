...
export default function ChartBar(props: any) {
  ...
  const handleData = (chartData: any) => {
    const { data = [], fields = [], name } = chartData;
  ...
  const setDashboardRules = (basicWidth: any, basicForceFit: any, smallScreenFit: any) => {
  ...
  const config = {
    ...
    label: {
      ...
      formatter: (value: any, ev: any) => {
        ...
        if (type === 'interval') {
          if (!value) return 0;
          if (value / maxHighFromInterval < 0.05) {
            return formatValue({
              value,
              total,
              lineBreak: true,
        ...
        return value / maxHighFromIntervalStack > 0.05
