...
export default (
  { casePercentage, taskPercentage, caseRemainingTime, taskRemainingTime, isUrgent, isSmallScreen }: any,
  size = 'normal'
) => {
  ...
  const { remainDays, remainHours, remainMins, remainSeconds } = getRemainingTime(remain);
  ...
  return (
    <div className={size === 'small' ? styles['remainingTime-small'] : styles.remainingTime}>
      <Progress
        className={`ant-progress-no-back ${styles.progress}`}
        type="circle"
