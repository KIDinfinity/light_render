...
let isFocusForOneSec = false, timeout: false | number = false;
const TaskTable = React.memo(({ getTaskDetail }: any) => {
  const [height, setHeight] = useState(window.innerHeight);
  const dispatch = useDispatch();
  const stateOfSearch = useSelector((state: any) => state.advancedQueryController.stateOfSearch);

  const resultField = useSelector((state) => state.configController?.configuration?.[state.homeList?.filter]?.resultField) || []
  ...
  const moduleFocus = useSelector((state: any) =>
  ...
  return (
    <Table
      ...
      scroll={{
        y: height * 0.62 - 48 - 16 * 2 - 64 - 45 - 8 + (height < 626 ? 21 : 0),
        x: 'max-content',
