...
class List extends Component<IComponentProps> {
  ...
  hotKeyChange = (isNext) => {
    ...
    let selectedIndex = flowData.findIndex(flow => flow.caseCategoryName === currentCaseCategory?.caseCategoryName)
    if(selectedIndex === -1)
      return;
    if (isNext) {
  ...
  render() {
    ...
    const selectedIndex = flowData.findIndex(flow => flow.caseCategoryName === currentCaseCategory?.caseCategoryName)
      ...
      <div className={styles.flow_loop}>
        ...
        {/* <div style={{ position: 'absolute', top: 245, left: 275, backgroundColor: 'red', width: 10, height: 10, borderRadius: 10, transform: 'translate(-50%, -50%)' }}/> */}
        {
          finalFlowData.map((circle, index) => {
            ...
            const width = isSelected? '110px' : '100px';
            const height = width;
            ...
            let opacity = 0, visibility = 'visible';
            ...
            return (
              <div
                ...
                >
                <div className={classnames(styles.ballInnerContainer, styles.center)} style={{ rotate: -rotate + 'deg'}}>
                  <div className={classnames(styles.flowIndicateCircle, styles.center)}
                    style={{ background: `conic-gradient(${circle.gradientColor.map(([color, percent]) => color + ' ' + percent).join(', ')})` }}>
                  ...
                  {circle.warning && <img src={Warning} alt="urgent" className={styles.warning} style={selectedIndex === index? { bottom: 'calc(50% - 8px)' } : { bottom: '23%' }} onClick={e => {
                    if (delta > 3)
                      return;
                    e.stopPropagation()
                ...
                <div className={styles.shade} style={selectedIndex === index? {backgroundColor: 'rgba(0,0,0,0)'} : void 0}/>
              </div>
            )
          })
        }
