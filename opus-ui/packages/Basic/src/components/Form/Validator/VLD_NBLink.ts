...
export const VLD_NBLink = (state: any, action: any) =>
  produce(state, (draftState: any) => {
    ...
    if (lodash.includes(config.clientDetail.matching, lodash.keys(changedFields)[0])) {
      lodash.map(config.clientDetail.list, ({ fieldId, errors }: any, index: number) => {
    ...
    if (lodash.includes(config.clientDetail.matching, lodash.keys(changedFields)[0])) {
      lodash.map(config.clientDetail.list, ({ fieldId, errors }: any, index: number) => {
        ...
        if (
          ...
          draftState.modalData.entities.clientMap[id].personalInfo[fieldId] = {
            value: cleanItem[fieldId],
            name: fieldId,
            errors: errors(),
            validating: false,
          ...
          draftState.modalData.entities.clientMap[id].personalInfo[fieldId] = cleanItem[fieldId];
        }
      });
    }
  });
