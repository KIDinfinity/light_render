...
const getValue = ({ target, state, namespace, form }: any) => {
...
const Rule = (configs: any, form: any, namespace: string) => {
    ...
    const getConditionsValue = (currentCondition: any) => {
      ...
      const values: any[] = lodash.map(lodash.compact(conditions), (condition: any) => {
        if (lodash.has(condition, 'conditions')) {
          ...
          const rightValue = getValue({ target: condition.right, state, namespace, form });
