...
export function saveDefaultPayeeV2(payee: any, changedFields?: any) {
  const keys = lodash.keys(changedFields);
  const defaultPayee = getPayeeDefaultData(payee);
  let merged: any = {};
  ...
  if (!lodash.isUndefined(changedFields) && keys.length === 1) {
    ...
    merged = lodash.merge(defaultPayee, {
      ...lodash.pick(payee, payeeOmitKeys),
      accountHolder:
        lodash.isString(payee?.accountHolder) && !lodash.isEmpty(payee?.accountHolder)
          ? payee?.accountHolder
          : payee?.payeeBankAccountList?.[0]?.accountHolder,
    });
  }
