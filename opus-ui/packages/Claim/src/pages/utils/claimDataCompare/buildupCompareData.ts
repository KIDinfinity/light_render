...
const buildupTreatmentPayable = (claimData: any) => {
  const payable = lodash.cloneDeep(claimData[ECompareModule.TreatmentPayable]);
  const monitorFields = CFileds.reloadData()?.[ECompareModule.TreatmentPayable];
  return lodash
    .chain(payable)
    .orderBy('viewOrder')
    .map((data: any) => {
...
const buildupClaimPayable = (claimData: any) => {
  const payable = lodash.cloneDeep(claimData[ECompareModule.ClaimPayable]);
  const monitorFields = CFileds.reloadData()?.[ECompareModule.ClaimPayable];
  return lodash
    .chain(payable)
    .orderBy('viewOrder')
    .map((data: any) => {
      ...
      return {
        ...pickMonitorFields(data, monitorFields),
        [ECompareModule.TreatmentPayable]: buildupTreatmentPayable(data),
      };
    })
    .compact()
    .value();
...
const buildupCompareData = (claimData: any) => {
  ...
  return {
    ...
    [ECompareModule.ClaimPayable]: buildupClaimPayable(claimData),
  };
};
