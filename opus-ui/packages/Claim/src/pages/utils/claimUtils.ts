...
const { dataMaping, getDictNameByCode, getDictCodeByName, getMedicalProvider } = new SeachCustom();

export { dataMaping, getDictNameByCode, getDictCodeByName, getMedicalProvider };
...
export function collectRemark(payableMap: any, claimPayable: any) {
  ...
  const remarkSubPayable = lodash
    ...
    .flatten()
    .map((payableItem: any) => payableItem?.remark?.split(/[;\n]+/))
    .flatten()
    .value();
