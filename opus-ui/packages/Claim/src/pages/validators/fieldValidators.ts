...
export const VLD_001175 = () => (_, value, callback) => {
  if (!/^\d{3}$/.test(value)) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001348' }));
  }
  callback();
};

export const VLD_001176 = () => (_, value, callback) => {
  const split = value?.length ? value.split(',') : [];

  if (lodash.some(split, (item) => !/^\d{5}$/.test(item))) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001349' }));
  }
  callback();
};

//校验只能输入：泰文、数字、字母、空格（校验opus th name相关）
export const VLD_001177 = () => (_, value, callback) => {
  const regex = /^[\u0E00-\u0E7Fa-zA-Z0-9 ]+$/;

  if (value && !regex.test(value)) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001354' }));
  }

  callback();
};

//校验只能输入：泰文、数字、字母、空格 - /（校验opus th address1相关）
export const VLD_001178 = () => (_, value, callback) => {
  // 增加了 - 和 /
  const regex = /^[\u0E00-\u0E7Fa-zA-Z0-9 \-\/]+$/;

  if (value && !regex.test(value)) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001354' }));
    return; // 建议加 return，避免多次 callback
  }

  callback();
};
//校验insured的idcard和payor的passport
export const VLD_001179 = (targetValue) => (_, value, callback) => {
  const idCardValue = String(formUtils.queryValue(targetValue));
  if (value === idCardValue) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001354' }));
  }
  callback();
};

export const VLD_001180 = () => (_, value, callback) => {
  // Email 正则表达式
...
export const VLD_001181 = () => (_, value, callback) => {
  const regex = /^[0-9]+$/; // 只允许数字

  if (value && !regex.test(value)) {
    callback(formatMessageApi({ Label_COM_Message: 'MSG_001354' }));
    return; // 避免多次 callback
  }

  callback();
};
//校验只能输入：泰文、数字、字母（校验opus th passportNo相关）
export const VLD_001182 = () => (_, value, callback) => {
  // 增加了 - 和 /
  const regex = /^[\u0E00-\u0E7Fa-zA-Z0-9]+$/;
...
//校验idCard
export const VLD_001183 = () => (_, value, callback) => {
