...
import SplitcaseIcon from '@/assets/SplitCase.svg';
import styles from './caseSplit.less';
import { Confirm, Close } from 'opus/Components/Modals';
...
class CaseSplit extends PureComponent<IProps, IStates> {
  ...
  get getFooterButton() {
    ...
    return [
      <Button key="cancel" onClick={this.fnCancel}>
        {formatMessageApi({
  ...
  fnConfirm = async () => {
    ...
    if (success) {
      ...
      this.setState({
        successVisible: true,
        newCaseNo: resultData?.newCaseNo,
      });
    } else {
          ...
          >
            {showContent && (
              <div className={styles.contentWrap}>
                <div className={styles.titleBox}>
                  <img src={SplitcaseIcon} />
                  <div className={styles.title}>
                    {formatMessageApi({ Label_CLM_Opus: 'splitCase' })}
                  </div>
                </div>
                <TabRadio
                  splitType={splitType}
            ...
            >
              <div className={styles.businessNoList}>
                {/* 需要国际化 */}
                {'Please confirm to continue to split selected treatments.'}
              </div>
            </Confirm>
            <Close
              className={styles.successModal}
              show={this.state.successVisible}
              handleCancle={() => {
                this.setState({
                  successVisible: false,
                });
              }}
              handleClose={this.fnCancel}
            >
              {/* 需要国际化 */}
              <div>Selected treatments have been successfully split and moved.</div>
              {this?.state?.newCaseNo && (
                <div
                  className={styles.newCaseNumber}
                >{`New Case Number is ${this.state.newCaseNo}`}</div>
              )}
            </Close>
          </Modal>
        )}
