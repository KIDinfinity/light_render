...
@connect(
  (
    ...
    diagnosisItem: apOfClaimCaseController.claimEntities.diagnosisListMap[diagnosisId],
...
class DiagnosisListItem extends Component {
  ...
  handleDelete = () => {
    ...
    dispatch({
      type: 'apOfClaimCaseController/removeDiagnosisItem',
      payload: {
        incidentId,
        diagnosisId,
      },
  ...
  searchIsCIByDiagnosisCode = (value) => {
    const { dispatch, diagnosisId }: any = this.props;
    dispatch({
      type: 'apOfClaimCaseController/checkIsCIByDiagnosisCode',
      payload: {
        diagnosisId,
        searchCode: value,
      },
      ...
      <CardOfClaim showButton={!taskNotEditable} handleClick={this.handleDelete}>
        <Form layout="vertical">
          <FormLayout json={diagnosisLayout}>
            ...
            />
            <FormItemCheckbox form={form} disabled formName="ci3" labelId="3CI" />
          </FormLayout>
        </Form>
      </CardOfClaim>
    );
