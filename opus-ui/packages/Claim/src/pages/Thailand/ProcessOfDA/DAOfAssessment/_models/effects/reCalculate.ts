...
export default function* reCalculate({}: any, { put, select }: any) {
  const { hasChangeSection, claimEntities, expectDecisionList: expectDecisionListO } = yield select(
    (state: any) => ({
      hasChangeSection: state?.daOfClaimAssessmentController?.claimProcessData?.hasChangeSection,
      expectDecisionList:
        state?.daOfClaimAssessmentController?.claimProcessData?.expectDecisionList,
