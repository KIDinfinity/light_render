  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (Object.keys(changedFields).length === 1 && lodash.has(changedFields, 'payTo')) {
      ...
      switch (formUtils.queryValue(changedFields.payTo)) {
        case 'NP':
          bankAccounts = draftState.claimProcessData.payeeList?.map(
            (payeeId) => draftState.claimEntities.payeeListMap?.[payeeId]
          );
          break;
        case 'PP':
          bankAccounts = draftState.claimProcessData.policyPayorBOList
            ?.map(
              (payorId) =>
                draftState.claimEntities.policyPayorBOListMap?.[payorId]?.claimBankAccounts || []
            )
            ?.flat();
          break;
        case 'PO':
          bankAccounts = draftState.claimProcessData.policyOwnerBOList
            ?.map(
              (ownerId) =>
                draftState.claimEntities.policyOwnerBOListMap?.[ownerId]?.claimBankAccounts || []
            )
            ?.flat();
          break;
      }
