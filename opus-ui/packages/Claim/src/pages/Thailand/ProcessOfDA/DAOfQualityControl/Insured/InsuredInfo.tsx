...
@connect(
  ({
    ...
    refreshLoading: loading.effects['daOfClaimCaseController/reloadInsureInfo'],
    insured: lodash.get(daOfClaimCaseController, 'claimProcessData.insured'),
...
@Form.create<IFormProps>({
  onFieldsChange(props, changedFields) {
    ...
    if (!shouldUpdateState(validating, changedFields)) return;
    // @ts-ignore
...
class InsuredInfo extends Component<IProps> {
  ...
  refreshInsuredInfo = async () => {
    ...
    (dispatch as Dispatch)({
      type: 'daOfClaimCaseController/reloadInsureInfo',
      payload: {
        caseCategory,
      },
    });
        ...
        >
          <Form layout="vertical">
            <FormLayout json={insuredLayout}>
              ...
              />
              {CaseCategory.TH_GC_CTG01 === caseCategory && (
                <FormItemInput
                  form={form}
                  disabled
                  formName="policyId"
