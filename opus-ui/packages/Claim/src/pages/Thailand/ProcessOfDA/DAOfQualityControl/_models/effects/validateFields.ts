...
export default function* validateFields(_, { select, put }: any) {
  ...
  const claimProcessData = yield select(
    (state: any) => state.daOfClaimCaseController.claimProcessData
  );
  const claimEntities = yield select((state: any) => state.daOfClaimCaseController.claimEntities);

  // const denormalizedData = denormalizeClaimData(claimProcessData, claimEntities);
  // let errors = formUtils.getErrorArray(denormalizedData);
