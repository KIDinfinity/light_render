...
export default (state: any, { payload }: any = {}) => {
  return produce(state, (draftState: any) => {
    ...
    const { payeeList, policyBenefitList } = draft.claimData;
    const exchangeRateList = lodash
      .chain(policyBenefitList[0].beneficiaryList)
      .map((benefit) => benefit.exchangeRateRecord)
      .compact()
      .map((exchangeRates) => safeParseUtil(exchangeRates))
      .compact()
      .flatten()
      .value();
