...
export default function* validateFields(action: any, { select, put, take }: any) {
  ...
  const controllerForms = yield select(({ formCommonController }: any) => formCommonController.forms);
  ...
  const formDatas: any[] = lodash.values(action?.payload?.useControllerForm? controllerForms : forms);
  let errors: any[] = [];
