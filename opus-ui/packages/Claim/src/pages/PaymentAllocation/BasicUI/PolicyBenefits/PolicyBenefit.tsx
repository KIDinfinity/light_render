...
const PolicyBenefit: FunctionComponent<IPolicyBenefit> = ({
  ...
  return (
    <div className={styles.PolicyBenefit}>
      ...
      {lodash.map(beneficiaryList, (beneficiaryItem: BeneficiaryModal, index: number) => {
        const sectionRef = React.createRef();
        const handleClose = (beneficiaryId?: string) => {
          deleteWarning({
            sectionRef,
            sectionID: SectionID.Beneficiary,
          }).then(() => {
            dispatch({
        ...
        return (
          <Panel.BackColor
            ...
            onClose={() => handleClose(beneficiaryItem.id)}
