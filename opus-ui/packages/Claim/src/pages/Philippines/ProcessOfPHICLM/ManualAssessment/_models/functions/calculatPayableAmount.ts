  ...
  if (lodash.isArray(claimPayableList) && claimPayableList.length > 0) {
    lodash.map(claimPayableList, (claimPayableItemId) => {
      ...
      if (formUtils.queryValue(claimPayableItem.claimDecision) === ClaimDecision.approve) {
        totlePayableAmount = add(
          totlePayableAmount,
          formUtil.queryValue(claimPayableItem.payableAmount)
        );
      } else if (
        formUtil.queryValue(claimPayableItem.claimDecision) === ClaimDecision.deny &&
        formUtil.queryValue(claimPayableItem.denyWithRescission)
      ) {
        totlePayableAmount = add(
          totlePayableAmount,
          formUtil.queryValue(claimPayableItem.payableAmount)
        ...
        totlePayableAmount = add(
          totlePayableAmount,
          formUtil.queryValue(claimPayableItem.systemCalculationAmount)
        );
      }
    });
  }
