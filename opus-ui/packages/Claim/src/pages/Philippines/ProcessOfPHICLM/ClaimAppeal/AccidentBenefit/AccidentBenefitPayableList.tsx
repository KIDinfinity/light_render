...
const mapStateToProps = (
  ...
  { treatmentPayableId, withData: { caseType } }: any
) => {
  ...
  const treatmentPayableItem = lodash.get(
    claimEntities,
    `treatmentPayableListMap.${treatmentPayableId}`
  );
  ...
  return {
    ...
    accidentBenefitPayableListMap: claimEntities.accidentBenefitPayableListMap,
  };
};
...
class AccidentBenefitPayableList extends Component {
  ...
  handleAdd = () => {
    ...
    dispatch({
      type: 'PHCLMOfAppealCaseController/addAccidentBenefitPayableItem',
      payload: {
        idAssemble,
      },
    });
...
export default withContextData(AccidentBenefitPayableList);
