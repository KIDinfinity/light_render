...
const Declarations = (props: IProps) => {
  ...
  const isRadioNotEditable = taskNotEditable || !(form.getFieldValue('taxDeclarationsFlag') || formUtils.queryValue(usTaxInformation?.taxDeclarationsFlag));

  const isNotEditable = taskNotEditable || ![TaskDefKey.PH_POS_ACT001, TaskDefKey.PH_POS_ACT003].includes(taskDefKey);
...
export default connect(
  ({ formCommonController, claimEditable, GeneralPOSPHNotCFTController, processTask }: any) => ({
    taskNotEditable: claimEditable?.taskNotEditable,
    usTaxInformation:
      GeneralPOSPHNotCFTController?.claimProcessData?.businessData?.transactionTypes?.[0]?.usTaxInformation || {},
