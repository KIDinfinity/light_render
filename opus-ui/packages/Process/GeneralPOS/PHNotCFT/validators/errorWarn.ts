...
// checked，不能都不填
export const VLD_000289 = (usTaxInformation: any) => {
  const usTax = formUtils.cleanValidateData(usTaxInformation);

  return usTax?.taxDeclarationsFlag && !usTax?.cardNo && !usTax?.identificationNo ? [{}] : [];
};
