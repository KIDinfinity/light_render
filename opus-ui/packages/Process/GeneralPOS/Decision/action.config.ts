...
export default {
  submit: {
    validate: async ({ dispatch }: any) => {
      ...
      if (result?.result) {
        const filterDefault = result.errorMsg || [].filter((item) => item !== 'MSG_000828');
        if (filterDefault.length > 0) {
          filterDefault.forEach((item: any) => {
            if (item) {
              notification.error({
                message: formatMessageApi({ Label_COM_WarningMessage: item }),
              });
            }
          });
        } else {
          notification.error({
            message: formatMessageApi({ Label_COM_WarningMessage: 'MSG_000828' }),
          });
        }
    ...
    anyway: async ({ responseCollect, dispatch, taskDetail }) => {
      try {
        if (
