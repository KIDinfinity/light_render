...
const CustomerInfo = () => {
  const { mainPayorClientId, clientInfoList, policyClientRoleList, clientContactList } =
  ...
  const info = lodash.find(clientInfoList, (item) => item.clientId === mainPayorClientId) || {};
  const contactInfo =
    lodash.find(clientContactList, (item) => item.clientId === mainPayorClientId) || {};

  const roles: any[] = useMemo(
    () =>
      lodash
        .chain(policyClientRoleList)
        .filter((item) => item.clientId === mainPayorClientId && item?.policyId === mainPolicyId)
        .uniqBy('role')
        .value(),
    [mainPayorClientId, mainPolicyId, policyClientRoleList]
  );
  ...
  return (
    <div className={styles.customerInfo}>
      <div className={classNames(styles.nameRole, styles.autoSpace)}>
        ...
        <ShowButton clientId={mainPayorClientId} />
      </div>
