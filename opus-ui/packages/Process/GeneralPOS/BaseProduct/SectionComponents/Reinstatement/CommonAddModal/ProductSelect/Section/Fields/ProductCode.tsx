...
export const FormItem = ({ isShow, layout, form, editable, field, config, transactionId }: any) => {
  ...
  const productCodeList = useSelector(
    ({ [NAMESPACE]: modelnamepsace }: any) => modelnamepsace.dictObject?.productCodeList
  );
      ...
      .filter(
        (item) =>
          formUtils.queryValue(item.uwCoverageDecision?.decision) ===
            BenefitLevelDecisionEnum.NonStandard && item?.clientId === name
      )
      .map((item) => ({
        dictCode: item?.productCode,
        dictName: productCodeList?.[item?.productCode]?.productName || item?.productCode,
      })),
    'dictCode'
  );
