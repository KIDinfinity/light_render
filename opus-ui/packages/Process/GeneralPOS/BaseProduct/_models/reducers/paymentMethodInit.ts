...
export default (state: any, { payload }: any) =>
  produce(state, (draftState: any) => {
    ...
    const {payoutPaymentMethod} = lodash.find(lodash.get(draftState, 'processData.policyInfo.policyInfoList', []), i => i.policyId === mainPolicyId) || {}
    ...
    if (
      ...
      lodash.set(draftState, `${transactionPath}.paymentMethodList[0].payoutOption`, payoutPaymentMethod || OptionEnum.BTR);
      return;
    }
    ...
    const originClientBankAccountList = lodash
      ...
      ?.reduce((res, i) => {
        if(payoutPaymentMethod == OptionEnum.BTR && i.preferPaymentFlow == 'O' && i.policyId == mainPolicyId){
      ...
      ?.map((bankItem, itemIndex) => ({
        ...
        onlyKey: `${bankItem.bankCode}${bankItem.bankAccountNo}`,
      }))
      ?.uniqBy('onlyKey')
      .value();
    ...
    if (lodash.isEmpty(paymentMethodList)) {
      ...
      const originClientBankCodes = originClientBankAccountList.map(
        (item) => `${item?.bankCode}${item?.bankAccountNo}`
      );
      ...
      const newAccountList = originClientBankAccountList.map((item, index) => {
        const paymentItem = txPmBankList.find((paymentItem) => {
          ...
          return (
            ...
            `${cleanData?.bankCode}${cleanData?.bankAccountNo}` ===
      ...
      const filterTxPmBankList = txPmBankList.filter((paymentItem) => {
        const cleanData = formUtils.cleanValidateData(paymentItem);
        return !originClientBankCodes.includes(`${cleanData?.bankCode}${cleanData?.bankAccountNo}`);
      });
