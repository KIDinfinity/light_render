...
const updateTypeFn = {
  ...
  [OperationTypeEnum.UPDATE]: (state, cf, otherParams) => {
    const { transactionPath, validating, changeTypeIndex, clientIndex, otherData, section } =
      otherParams;
  ...
  [OperationTypeEnum.LISTINFOUPDATE]: (state, cf, otherParams) => {
    const { transactionPath, validating, changeTypeIndex, clientIndex, otherData, changeType } =
      otherParams;
