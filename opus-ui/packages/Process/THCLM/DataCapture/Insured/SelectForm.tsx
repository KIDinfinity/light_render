...
import { useGetPolicyList } from '../../_hooks';
...
const tableColumns = [
  ...
  {
    title: formatMessageApi({
      Label_BIZ_Policy: 'Policy Source',
    }),
...
const SelectInsuredForm = () => {
  ...
  const policyList = useGetPolicyList({ NAMESPACE });
  ...
  const handleSelectInsured = async () => {
    ...
    const { policyResultList }: any = selectColumns;
    await dispatch({
  ...
  return (
    <div className={searchStyles.selectInsured}>
      <Table
        rowSelection={{
          type: 'radio',
          selectedRowKeys: [selectColumns?.policyId],
          onSelect: (record) => {
