...
const relationshipForBenefitType = ({ target, mapKeys }: any) => {
  ...
  return {
    ...
    payableAmount: subtract(
      ...
      lodash.reduce(
        ...
        (result, reduceItem: any) =>
          add(result, formUtils.queryValue(reduceItem?.deductibleNetExpense)),
        0
      )
    ),
    ...
    calculationAmount: lodash.reduce(
      extra?.children,
      (result, reduceItem: any) => add(result, formUtils.queryValue(reduceItem?.calculationAmount)),
      0
    ),
