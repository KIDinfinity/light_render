...
import stashConfirmedSustainabilityCheckingSelected from 'process/NB/ManualUnderwriting/_models/reducers/stashConfirmedSustainabilityCheckingSelected';

describe('stashConfirmedSustainabilityCheckingSelected', () => {
  test('set confirmedSustainabilityCheckingSelected ', () => {
    const state = {
    ...
    const renderer = renderHook(() => {
      return stashConfirmedSustainabilityCheckingSelected(state);
    });

    expect(renderer.result.current).toEqual({
      ...state,
      confirmedSustainabilityCheckingSelected: 'RT',
    });
  });
});
