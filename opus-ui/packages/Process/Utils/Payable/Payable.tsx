...
export default {
  ...
  addCashlessPayable: (params: any) => {
    const { claimEntities, claimProcessData, extra } = params;
    const { incidentId, treatmentId } = extra;
  ...
  // 获取新增C类型 OP的payable
  addCashlessOPTreatmentPayable: (params: any) => {
    const { incidentId, treatmentId, claimEntities, claimProcessData, extra } = params;
    const { claimPayableListMap, treatmentListMap, treatmentPayableListMap, invoiceListMap } =
      claimEntities;
    const { claimNo } = claimProcessData;
    ...
    return {
      ...
      treatmentPayableItem: !sourceTreatmentPayableId
        ...
        : {},
      opTreatmentPayableItem: addOPTreatmentPayable({
        claimNo,
        id: uuidv4(),
        incidentId,
        treatmentId,
        opTreatmentId: extra.opTreatmentId,
        payableId,
        treatmentPayableId,
        extra: { ...extra },
      }),
  ...
  addReimbursementPayable: (params: any) => {
    const { payableAmount, incidentId, treatmentId, serviceItemId } = params.extra || {};
    const { claimEntities, claimProcessData } = params;
  ...
  // 获取新增MIC类型payable
  addMajorIllnessCashPayable: (params: any) => {
    const { claimEntities, claimProcessData, extra } = params;
    const { incidentId } = extra;
    const { claimPayableListMap } = claimEntities;
    const { claimNo } = claimProcessData;
    ...
    return {
      ...
      claimPayableItem: !claimPayableItem?.id
        ? addClaimPayable({ id: payableId, claimNo, incidentId, claimPayableListMap, extra })
        : claimPayableItem,
      claimIncidentPayableItem: addClaimIncidentPayable({
        claimNo,
        id: uuidv4(),
        payableId,
        incidentId,
  ...
  // 更新L类型数据
  updateLifePayableData: ({ draftState, updateFields, extra }: any) => {
    const { claimPayableId, payableAmount } =
      lodash.pick(extra, updateFields.concat(['claimPayableId'])) || {};

    const updatePayableItem =
      draftState?.claimEntities.claimPayableListMap?.[claimPayableId]?.lifePayable;
  ...
  // 更新MIC类型数据
  updateMajorIllnessCashPayableData: ({ draftState, updateFields, extra }: any) => {
    const { claimIncidentPayableId, payableAmount } =
      lodash.pick(extra, updateFields.concat(['claimIncidentPayableId'])) || {};
  ...
  // 更新C OP类型数据
  updateOPTreatmentPayableData: ({ draftState, updateFields, extra }: any) => {
    const { id, payableAmount, payableDays } =
      lodash.pick(extra, updateFields.concat(['id'])) || {};

    const updatePayableItem = draftState?.claimEntities.opTreatmentPayableListMap?.[id];
  ...
  updateProcedurePayableData: ({ draftState, updateFields, extra }: any) => {
    const { procedurePayableId, payableAmount, reimbursementMultiple } =
  ...
  updateOtherProcedurePayableData: ({ draftState, updateFields, extra }: any) => {
    console.log('updateOtherProcedurePayableData: ');
  ...
  changeAddData({ draftState, addData }: any) {
    ...
    // 设置claimIncidentPayable数据
    if (claimIncidentPayableItem && !lodash.isEmpty(claimIncidentPayableItem)) {
      const claimIncidentPayableList =
        draftState?.claimEntities?.claimPayableListMap?.[payableId]?.claimIncidentPayableList || [];
      // eslint-disable-next-line no-param-reassign
      draftState.claimEntities.claimPayableListMap[payableId] = {
        ...draftState?.claimEntities?.claimPayableListMap?.[payableId],
        claimIncidentPayableList: lodash.uniq([
          ...claimIncidentPayableList,
          claimIncidentPayableItem?.id,
        ]),
      };

      // eslint-disable-next-line no-param-reassign
      draftState.claimEntities.claimIncidentPayableListMap[claimIncidentPayableItem?.id] = {
        ...claimIncidentPayableItem,
      };
    }
    ...
    // 设置opTreatmentPayable数据
    if (opTreatmentPayableItem && !lodash.isEmpty(opTreatmentPayableItem)) {
      const treatmentPayableItemMatched =
        draftState?.claimEntities?.treatmentPayableListMap?.[
          opTreatmentPayableItem?.treatmentPayableId
        ];

      const id = opTreatmentPayableItem.id;
      treatmentPayableItemMatched.opTreatmentPayableList = lodash.uniq([
        ...treatmentPayableItemMatched.opTreatmentPayableList,
        id,
      ]);

      // eslint-disable-next-line no-param-reassign
      draftState.claimEntities.opTreatmentPayableListMap[id] = opTreatmentPayableItem;
    }
