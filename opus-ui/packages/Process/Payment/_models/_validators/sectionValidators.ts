...
import { validatorPolicyBenefit, validatorPayee, validatorBeneficiary } from './validatorData';
...
const totalPayableAmountDiff = (policyBenefitList = [], allclaimPayableList, claimDecision) => {
  const totalPolicyBenefitAmount = getTotalBenefitAmount(policyBenefitList);
  const claimPayableAmount = getTotalClaimPayableAmount(allclaimPayableList);

  return (
    claimDecision.totalPayableAmount !== claimPayableAmount ||
    claimDecision.totalPayableAmount !== totalPolicyBenefitAmount
  );
};
...
const policyNoTotalPayableAmountDiff = (allclaimPayableList, beneficiaryList, policyNo) => {
  ...
  const benefitAmounts = +lodash
    ...
    .reduce((total, beneficiary: BeneficiaryModal) => {
      return add((formUtils.queryValue(beneficiary.beneficiaryAmount) as number) || 0, total);
    }, 0)
    .value();
...
export const VLD_000334 = (curPolicyBenefit: PolicyBenefitModal, claimPayableList: any[]) => {
  ...
  output.result =
    totalPayableAmountDiff(policyBenefitList, allclaimPayableList, claimDecision) ||
    policyNoTotalPayableAmountDiff(allclaimPayableList, beneficiaryList, policyNo);
...
export const VLD_000940 = (
  ...
  const res = lodash.find(claimPayableList, (payable) => {
    const payableAmount = formUtils.queryValue(payable.payableAmount);
    const serviceItemList = lodash.values(serviceItemPayableListMap);

    const childPayable = {
      [eBenefitCategory.Reimbursement]: serviceItemList.filter(serviceItem => serviceItem.payableId === payable.id ),
      [eBenefitCategory.Cashless]: payable.treatmentPayableList,
      [eBenefitCategory.Life]: [payable.lifePayable]
    }[payable.benefitCategory];
