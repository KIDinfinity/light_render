...
const saveDiagnosisItem = (state: any, action: any) => {
  ...
  const nextState = produce(state, (draftState: any) => {
    ...
    if (
      ...
      tempDiagnosis.criticalIllness = formUtils.queryValue(changedFields.criticalIllness) ? 1 : 0;
      // TODO 如何保留上次输入
    ...
    if (lodash.has(changedFields, 'diagnosisName') && lodash.size(changedFields) === 1) {
      const treatmentList =
        draftState?.claimEntities?.incidentListMap?.[tempDiagnosis?.incidentId]?.treatmentList;
