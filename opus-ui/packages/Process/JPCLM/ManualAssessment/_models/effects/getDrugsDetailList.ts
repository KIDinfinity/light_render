...
export default function* getDrugsDetailList({ payload }: any, { call, select, put }: any) {
  const filterParams = yield select((state: any) => state.JPCLMOfClaimAssessment.DrugsDetail.filterParams);
  const searchState: any = yield select((state: any) => state.JPCLMOfClaimAssessment.DrugsDetail.searchState);
  let responseInfo = [];
