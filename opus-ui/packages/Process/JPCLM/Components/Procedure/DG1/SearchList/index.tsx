...
export default (props: any) => {
  ...
  const DrugsDetail =
    useSelector(({ JPCLMOfDataCapture }: any) => JPCLMOfDataCapture.DrugsDetail) || {};
  const antiCancerProcedureItem = useSelector(
    ({ JPCLMOfDataCapture }: any) =>
      JPCLMOfDataCapture.claimEntities.otherProcedureListMap[otherProcedureId]
    ...
    const t = async () => {
      if (DrugsDetail.show) {
        if (lodash.isEmpty(DrugsDetail.allList)) {
          await dispatch({
            type: 'JPCLMOfDataCapture/getDrugsDetailList', //请求数据
            payload: {
              currentPage: 1,
            },
          });
  ...
  const handleCancel = () => {
    dispatch({
      type: 'JPCLMOfDataCapture/updateDrugsDetailList', //关闭并清空
      payload: {
  ...
  const handleConfirm = async () => {
    ...
    await dispatch({
      type: 'JPCLMOfDataCapture/saveDrugsDetailList', //保存已选中药品
      payload: {
        therapeuticMonth,
        otherProcedureId,
        changedFields: {
