    ...
    >
      {
        item.isPayableAdjusted && (
          <AdjustTreatmentPayable item={item} />
        )
      }
...
export default connect(({ formCommonController, [NAMESPACE]: modelnamespace }: any, { item }) => ({
  ...
  isPayableEditable: modelnamespace.claimEntities?.claimPayableListMap?.[item?.payableId]?.isNewPayable || !modelnamespace.claimProcessData?.appeal
}))(
  Form.create<any>({
