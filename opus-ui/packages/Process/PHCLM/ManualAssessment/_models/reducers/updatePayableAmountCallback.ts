  ...
  const nextState = produce(state, (draftState) => {
    ...
    const claimEntities = isAppeal
      ? {
          ...
          claimPayableListMap: lodash.pickBy(
            draftState.claimEntities.claimPayableListMap,
            (claimPayable) => claimPayable.isNewPayable || claimPayable.isPayableAdjusted
          ),
        }
      : draftState.claimEntities;

    const claimPayableList = isAppeal
      ? draftState.claimProcessData.claimPayableList?.filter(
          (claimPayableId) => claimEntities.claimPayableListMap?.[claimPayableId]
        )
