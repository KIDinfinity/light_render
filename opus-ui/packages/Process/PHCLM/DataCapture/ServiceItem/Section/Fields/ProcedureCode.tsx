...
export const FormItem = ({
  ...
  const getProductName = (procedureCode: any) => {
    return lodash.find(surgeryProcedureByRegion, (item: any) => {
      return item.surgeryCode === formUtils.queryValue(procedureCode);
    });
  };
  ...
  const Options = lodash
    ...
    .map((item: any) => {
      return {
        dictCode: item.id,
        dictName: `${formUtils.queryValue(item.procedureCode)}-${
          getProductName(formUtils.queryValue(item.procedureCode))?.surgeryName || ''
