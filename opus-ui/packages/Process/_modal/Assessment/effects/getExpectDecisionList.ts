  ...
  contrast: {
    ...
    datas: {
      ...
      assessObjectIdKeys: {
        ...
        [eBenefitCategory.Life]: 'incidentId',
        COP: 'opTreatmentId',
      },
    },
    // 匹配的filed
  ...
  return lodash
    ...
    .reduce((map: any, key: any) => {
      const list = lodash
        ...
        .reduce((datas: any, item: any) => {
          if (lodash.isPlainObject(item)) {
            return [
              ...datas,
              ...(lodash
                .chain(lodash.values(claimEntities?.[item?.path] || {}))
                .reduce(
                  (childDatas: any, childItem: any) =>
                    !!childItem?.[item.key] ? [...childDatas, childItem?.[item.key]] : childDatas,
                  []
                )
                .value() || []),
            ];
          }
          return !!lodash.values(claimEntities[item])
            ? [...datas, ...lodash.values(claimEntities[item] || {})]
            : datas;
        }, [])
        .value();
      return {
        ...
        ...(lodash
          ...
          .reduce((maps1: any, item: any) => {
            ...
            let benefitCategory = claimpayableItem.benefitCategory;
            if (
              claimpayableItem.benefitCategory === eBenefitCategory.Cashless &&
              claimpayableItem.benefitSubCategory === BenefitSubCategory.OP
            ) {
              benefitCategory = 'COP';
            }

            if (benefitCategory === key) {
              const newItem = formUtils.cleanValidateData({
                ...lodash.pick(item, [...fileds.basic]),
                ...(lodash
                  .chain(fileds.extra[benefitCategory] || [])
                ...
                assessObjectId: item[datas.assessObjectIdKeys[benefitCategory]],
                originAssessObjectId: item.originAdjustmentId,
                assessorOverrideCalculationDays: fileds.getPayableDay({ item }),
                assessorOverrideTimes: fileds.getAssessorOverrideTimes({ item }),
                ...fileds.getPayableAmount({ key: benefitCategory, item }),
              });
