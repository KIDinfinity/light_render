...
const ApplytoPolicies = ({ config, id }: any) => {
  ...
  const dataSource = policySortList.filter((mapId: any) => mapId.split('____')[1] !== mainPolicyNo);
  ...
  const check = lodash.size(applyToPolicyBOList) >= lodash.size(policySortList);

  const tableDisabled = /complete/i.test(taskStatus) || !editable;

  useEffect(() => {
    if (!/complete/i.test(taskStatus) && applyTo !== undefined) {
      dispatch({
  ...
  }, [servicingInit, applyTo, transactionTypeLevel]);
