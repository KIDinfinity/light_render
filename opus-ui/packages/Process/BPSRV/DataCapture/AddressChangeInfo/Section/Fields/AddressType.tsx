...
export const FormItem = ({ isShow, layout, form, editable, field, config }: any) => {
  ...
  const isESubmission = caseCategory === 'BP_SRV_CTG002';

  const visibleConditions = true;
  const editableConditions = !Rule(fieldProps['editable-condition'], form, '') && !isESubmission;
  const requiredConditions = Rule(fieldProps['required-condition'], form, '') && !isESubmission;
...
const AddressType = ({ isShow, layout, form, editable, section, id }: any) => (
  <Authority>
...
AddressType.displayName = localFieldConfig.field;
