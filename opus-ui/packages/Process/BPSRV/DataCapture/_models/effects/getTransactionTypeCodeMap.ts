...
export default function* getTransactionTypeCodeMap(_: any, { put, select, call }: any) {
  ...
  if (lodash.isEmpty(transactionTypeCodeMap)) {
    const caseCategory: string = yield select(
    ...
    if (lodash.isPlainObject(result) && result.success && lodash.isArray(result.resultData)) {
      yield put({
        ...
        payload: {
          transactionTypeCodeMap: result.resultData,
        },
      });
    }
  }
