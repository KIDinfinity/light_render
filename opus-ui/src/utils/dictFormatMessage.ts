...
import TypeCodeEnum from '@/enum/TypeCodeEnum';
...
function formatMessageApiTypeCodeLabel_CLM_Opus(dictCode: string, ...arg: any) {
  return formatMessageApi({ [TypeCodeEnum.Label_CLM_Opus]: dictCode }, ...arg);
}
 ...
 * @param data 需要过滤的数组
 */
function filterDataByLanguage(data:any) {
  if(data) {
    const language = tenant.getLocaleLang();
    return data.filter((d:any) => d.language === language) || [];
  } else {
    return [];
  }
}
...
export {
  ...
  formatMessageApi,
  formatMessageApiTypeCodeLabel_CLM_Opus,
