...
function Index({ type, children, claimNo, caseDetailCaseNo }) {
  ...
  let getHiddenCallback = {
    information: state => {
      return state.authController.noPermissionCases[formUtils.queryValue(state.navigatorInformationController.informationData?.caseNo)]
    },
    envoy: state => {
      return state.authController.noPermissionCases[formUtils.queryValue(state.envoyController.caseNo)]
    },
    360: state => {
      const changeId = state.insured360?.taskInfo?.changeId || ''
      return state.authController.noPermissionClaimNos[changeId] || state.authController.noPermissionCases[changeId]
    },
    caseDetail: state => {
  ...
  // 未知的权限类别默认不隐藏
  if(!getHiddenCallback)
    getHiddenCallback = () => false;
