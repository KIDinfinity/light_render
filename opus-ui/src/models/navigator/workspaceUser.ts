  ...
  effects: {
    advancedQuery: [
      function* advancedQuery({ payload }, { call, put, select }) {
        ...
        const mode = lodash.get(otherPayload, 'params.mode', 'table');
    ...
    *getUserOrganization({ signal = null }, { call, put, select, take }) {
      ...
      if (response?.success) {
        ...
        if(tenant.isTH()) {
          result = result.filter(item => item.organizationCode.includes('ORG_TH_NB'))
        }
        const userOrganization = organization(result);
        yield put({
          type: 'saveUserOrganization',
          payload: {
            userOrganization,
            userOrganizationCodeList:
              result?.map?.((item) => item.organizationCode) || [],
          },
        });
      }
    },
