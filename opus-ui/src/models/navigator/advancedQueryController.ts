...
export default {
  ...
  effects: {
    ...
    *getTransactionTypeList({ payload }: any, { call, put }: any) {
      ...
      const response = yield call(listTransactionTypesByCaseCategory, {
        caseCategory: categoryCode,
      });
      ...
      if (response?.success) {
        yield put({
          ...
          payload: {
            transactionTypeList: lodash.map(response.resultData, (item) => ({
              dictCode: item?.transactionTypeCode,
              dictName: formatMessageApi({
                Dropdown_SRV_TransactionType: item?.transactionTypeCode,
              }),
            })),
          },
        });
