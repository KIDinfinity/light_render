...
export default {
  ...
  effects: {
    *getAssigneeList({ payload }, { select, call, put, take }) {
    ...
    *getTargetAssignee({ payload }, { call, select }) {
      const { group } = payload;
      const { activityKey, caseCategory, businessNo, taskId } = yield select(
    ...
    *assignTask({ payload }, { call }) {
      ...
      return yield call(bpmProcessTaskService.assignTask, {
        assignee,
        assigner,
        taskId,
        caseCategory,
    ...
    *historyListener(_: any, { select, takeLatest, put }: any) {
      const isAssigneeListVisible = yield select(
        (state) => state.contactsAssigneeList?.isAssigneeListVisible
      );
