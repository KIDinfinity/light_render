...
import { getDrowDownList, filterDataByLanguage } from '@/utils/dictFormatMessage';
...
export default {
  ...
  effects: {
    ...
    *partOfBodyInjuredDropdown({ payload }: any, { call, put }: any) {
      const response = yield call(
    ...
    *diagnosisDropdown({ payload }: any, { call, put }: any) {
      const response = yield call(
    ...
    *medicalProviderDropdown({ payload }: any, { call, put }: any) {
      const response = yield call(
    ...
    *surgeryProcedureDropdown({ payload }: any, { call, put }: any) {
      const response = yield call(
    ...
    *serviceItemDropdown({ payload }: any, { call, put }: any) {
      const response = yield call(
      ...
      if (success && lodash.isArray(resultData)) {
        yield put({
          ...
          payload: {
            ...
            parentFieldName,
            subDicts: lodash
              .chain(filterDataByLanguage(resultData))
              .filter((item) => {
    ...
    *loadMutipleDictsByParentCode({ payload }: any, { call, put }: any) {
      const { parentCode, parentFieldName } = lodash.pick(payload, [
        'parentCode',
        'parentFieldName',
      ]);
      ...
      // 调用接口并传递数组形式的参数
      const response = yield call(miscCommonHierarchyLinkCommonMultiple, payload);
      const { success, resultData } = lodash.pick(response, ['success', 'resultData']);
      if (success && lodash.isArray(resultData)) {
        yield put({
          type: 'setCustomerRoleCodes',
          payload: {
            parentCode,
            parentFieldName,
            subDicts: lodash
  ...
  reducers: {
    ...
    setCustomDictsLoaded(state: any, action: any) {
      const { parentCode, parentFieldName, loaded } = lodash.pick(action?.payload, [
        'parentCode',
        'parentFieldName',
        'loaded',
      ...
      const nextState = produce(state, (draftState: any) => {
        if (!/[0-9]{1,}/g.test(parentCode)) {
          lodash.set(draftState, `customDictsLoaded.${parentFieldName}.${parentCode}`, loaded);
        }
      });
      return {
        ...nextState,
    ...
    setCustomerRoleCodes(state: any, action: any) {
      const { parentCode, parentFieldName, subDicts } = lodash.pick(action?.payload, [
        'parentCode',
        'parentFieldName',
        'subDicts',
      ...
      const nextState = produce(state, (draftState: any) => {
        lodash.set(draftState, `customerRoleDicts.${parentFieldName}.${parentCode}`, subDicts);
      });
      return { ...nextState };
    },
  },
