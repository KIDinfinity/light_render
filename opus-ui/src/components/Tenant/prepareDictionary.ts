...
const retryCallbacks = async (
  region: string,
  callbacks: (() => Promise<false | object>)[] = [],
  existingDicts = {},
  retryTimes = 3
) => {
  const retryHelper = async (
    callback: () => Promise<false | object>,
    remainingTimes: number = retryTimes
...
export const prepareLoginedDictionary = async () => {
  ...
  const companyCode = currentUser.companyCode || ['2', '3'];
  ...
  if (lodash.isEmpty(dictionary)) {
    ...
    const [labelDictionary, specialPermissionDictionary, dropdownDictionary, otherDictionary] =
      resultArr;
    ...
    const failedDictionaryArr = resultArr
      .map((result, index) => (result ? false : index))
      .filter((index) => index !== false);

    if (failedDictionaryArr.length) {
      retryCallbacks(
        region,
        failedDictionaryArr.map((index) => callbackArr[index]),
        saveDictionary
      );
    } else {
